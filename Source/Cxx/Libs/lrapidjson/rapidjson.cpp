...
private:
	...
	void encodeValue(lua_State* L, Writer* writer, int idx, int depth = 0)
	{
		...
		case LUA_TNUMBER:
			if (luax::isinteger(L, idx, &integer))
				...
				if (std::isnan(value)) {
					printf("warning!! json encode double is nan \n");
					value = 0;
				}

				if (!writer->Double(value)) {
					char s[100];
					sprintf(s, "error while encode double value %.2f", value);
					luaL_error(L, s);
				}
			}
			return;
	...
	/*
	Fixed:在遍历时使用lua_tolstring会改变index位置的值
	...
	void encodeObject(lua_State* L, Writer* writer, int depth)
	{
		...
		while (lua_next(L, -2))
		{
			...
			if (type == LUA_TSTRING)
			{
				...
				std::string key_ = std::to_string(key);
				writer->Key(key_.c_str(), key_.size());
				encodeValue(L, writer, -1, depth);
			}
			else if (type == LUA_TBOOLEAN) {
				bool key = lua_toboolean(L, -2);
				std::string key_ = key?"true":"false";
				writer->Key(key_.c_str(), key_.size());
				encodeValue(L, writer, -1, depth);
			}
			// pop value, leaving original key
...
static const struct luaL_Reg lib_json[] = {
	{ "encode", json_encode },
	{ "decode", json_decode },
	{ NULL, NULL }
};
...
{
	luaL_register(L, "json", lib_json);
	return 0;
}

LUALIB_API int luaopen_rapidjson2(lua_State* L)
{
	lua_newtable(L); // [rapidjson]

	luax::setfuncs(L, methods); // [rapidjson]

	lua_pushliteral(L, "json"); // [rapidjson, name]
