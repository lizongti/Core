...
class NotificationService : 
	...
	public Singleton<NotificationService>, 
	public LuaLib<NotificationService>
{
public:
	struct Config
	{
	...
	bool Init(const std::string& json)
	{
		...
		if(!ret)
		{
			LOG(SYS, ERROR) << boost::format("[NotificationService %x][%s] config json parse fail.\n") % this % __FUNCTION__ ;
			return false ;
		}
		...
		for (size_t index = 0; index < conf_.zeromqs.size(); ++index)
		{
			...
			if (!ret)
			{
				LOG(SYS, ERROR) << boost::format("[NotificationService %x][%s] message queue init fail.\n") % this % __FUNCTION__;
				return false;
			}
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[NotificationService %x][%s] lua loop init fail.\n") % this % __FUNCTION__;
			return false;
		}
	...
	virtual bool Start()
	{
		...
		for (size_t index = 0; index < zeromqs_.size(); ++index)
		{
			...
			if (!ret)
			{
				LOG(SYS, ERROR) << boost::format("NotificationService %x][%s] message queue start fail.\n") % this % __FUNCTION__;
				return false;
			}
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("NotificationService %x][%s] lua loop start fail.\n") % this % __FUNCTION__;
			return false ;
		}
	...
	virtual bool Stop()
	{
		...
		for (size_t index = 0; index < zeromqs_.size(); ++index)
		{
			...
			if (!ret)
			{
				LOG(SYS, ERROR) << boost::format("NotificationService %x][%s] message queue stop fail.\n") % this % __FUNCTION__;
				return false;
			}
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[NotificationService %x][%s] lua loop stop fail.\n") % this % __FUNCTION__;
			return false ;
		}
	...
	virtual bool Boot()
	{
		return Service<NotificationService>::Boot();
	}
