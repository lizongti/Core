#ifndef DBCACHE_CLIENT_SERVICE_HPP
#define DBCACHE_CLIENT_SERVICE_HPP

#include "../Base/BaseHeaders.h"
#include "./LoggerService.hpp"

class DbCacheClientService :
	public Service<DbCacheClientService>,
	public Singleton<DbCacheClientService>,
	public LuaLib<DbCacheClientService>
{
public:
	struct Config
	{
	...
	DbCacheClientService()
	{
		this->Depends(&LoggerService::Instance());
	}
	virtual ~DbCacheClientService()
	{
	...
	virtual bool Init(const std::string& json)
	{
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[DbCacheClientService %x][%s], config json parse fail.\n") % this % __FUNCTION__;
			return false;
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[DbCacheClientService %x][%s] message queue reader init fail.\n") % this % __FUNCTION__;
			return false;
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[DbCacheClientService %x][%s] message queue writer init fail.\n") % this % __FUNCTION__;
			return false;
		}
	...
	virtual bool Start()
	{
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[DbCacheClientService %x][%s] message queue reader start fail.\n") % this % __FUNCTION__;
			return false;
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[DbCacheClientService %x][%s] message queue writer start fail.\n") % this % __FUNCTION__;
			return false;
		}
	...
	virtual bool Stop()
	{
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[DbCacheClientService %x][%s] message queue reader stop fail.\n") % this % __FUNCTION__;
			return false;
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[DbCacheClientService %x][%s] message queue writer stop fail.\n") % this % __FUNCTION__;
			return false;
		}
	...
	virtual bool Boot()
	{
		return Service<DbCacheClientService>::Boot();
	}
	...
	{
		//cout << "DbCacheClientService WriteJson is:" << request << endl;
