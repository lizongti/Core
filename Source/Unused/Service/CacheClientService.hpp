#ifndef CACHE_CLIENT_SERVICE_HPP
#define CACHE_CLIENT_SERVICE_HPP

#include "../Base/BaseHeaders.h"
#include "./LoggerService.hpp"

class CacheClientService : 
	public Service<CacheClientService>, 
	public Singleton<CacheClientService>, 
	public LuaLib<CacheClientService>
{
public:
	struct Config
	{		
	...
	virtual bool Init(const std::string& json)
	{
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[CacheClientService %x][%s] config json parse fail.\n") % this % __FUNCTION__;
			return false;
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[CacheClientService %x][%s] message queue reader init fail.\n") % this % __FUNCTION__;
			return false;
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[CacheClientService %x][%s] message queue writer init fail.\n") % this % __FUNCTION__;
			return false;		
		}
	...
	virtual bool Start()
	{
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[CacheClientService %x][%s] message queue reader start fail.\n") % this % __FUNCTION__;
			return false;
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[CacheClientService %x][%s] message queue writer start fail.\n") % this % __FUNCTION__;
			return false;		
		}
	...
	virtual bool Stop()
	{
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[CacheClientService %x][%s] message queue reader stop fail.\n") % this % __FUNCTION__;
			return false;
		}
		...
		if (!ret)
		{
			LOG(SYS, ERROR) << boost::format("[CacheClientService %x][%s] message queue writer stop fail.\n") % this % __FUNCTION__;
			return false;		
		}
	...
	virtual bool Boot()
	{
		return Service<CacheClientService>::Boot();
	}
...
#endif // !CACHE_CLIENT_SERVICE_HPP
