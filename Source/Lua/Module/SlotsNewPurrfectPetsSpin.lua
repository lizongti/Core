...
--变量缓存
local Types = _G["NewPurrfectPetsTypeArray"].Types
...
--方法对象初始化
SlotsNewPurrfectPetsSpin = {}

do --Spin
    function SlotsNewPurrfectPetsSpin:Enter()
        --返回数据初始化
    ...
    function SlotsNewPurrfectPetsSpin:NormalSpin()
        --取出变量
        ----统一取出
        local player = self.parameters.player
        local player_game_info = self.parameters.player_game_info
        ...
        if need_gen_coin_item then
            ...
            if gen_coin_count > 0 then
                ...
                local lock_coin_info_arr =
                    (save_data.curr_spin_type == SPIN_ENUM.BASE_SPIN) and {} or
                    save_data.coin_spin_param.lock_coin_info_arr --已经锁定的金币信息(--base_spin中，不能使用之前coin_spin的lock信息)
        ...
        --统计信息
        --下次是否是base_spin
        special_parameter.is_next_base_spin = true
        --下次是否coin_spin
        special_parameter.is_next_coin_spin = true
        --Formation的名字
        special_parameter.formation_name = save_data.reel_info_arr[1].formation_name
        --reels_spin的统计
        special_parameter.reels_spin_param = {
        ...
        --coin_spin的统计
        special_parameter.coin_spin = {
            trigger_times_in_coin = 0,
            end_coin_count = nil, --结算时，金币的数量
            win_chip = 0 --赢钱
        }
        --游戏内jackpot统计
        special_parameter.game_jackpot = {
        ...
        --走地图统计
        special_parameter.can_enter_bonus_by_spin = false
        ...
        if need_gen_coin_info_arr then
            ...
            for result_idx, origin_result in ipairs(origin_result_arr) do
                ...
                local lock_coin_info_arr =
                    (save_data.curr_spin_type == SPIN_ENUM.BASE_SPIN) and {} or
                    save_data.coin_spin_param.lock_coin_info_arr
                --金币信息生成位置确定
        ...
        local have_take_jackpot = false --是否赢取了Jackpot
        ...
        local extra_payrate_ratio =
            CommonCal.Calculate.get_config(player, game_name .. "OthersConfig")[1].Base_Bet_Ratio
        --额外赔付系数
        ...
        if change_to_spin_type then --下次的spin类型发生改变
            ...
            elseif change_to_spin_type == SPIN_ENUM.COIN_SPIN then
                if is_agin_coin_spin then --再次coin_spin
                    ...
                    local play_again_num =
                        my_cal:GetAppointTypeNumInCoinArr(final_coin_info, COIN_PRIZE_ENUM.PLAY_AGAIN)
...
do --WalkBonus
    function SlotsNewPurrfectPetsSpin:BonusEnter()
        --取出变量
        local player = self.parameters.player
        local game_room_config = self.parameters.game_room_config
        ...
        if save_data.bonus_game_bouts > 0 then
            ...
            for item_idx, item_info in ipairs(special_item_arr_config) do --特殊格子获取
                ...
                client_map_info.special_item_arr[item_idx] =
                    my_cal:GetClinetMapSpecialItemInfo(item_info, item_use_info)
            end
    ...
    function SlotsNewPurrfectPetsSpin:BonusWalk()
        --取出变量
        local player_game_status = self.parameters.player_game_status
        local player = self.parameters.player
        local game_room_config = self.parameters.game_room_config
            ...
            if (turnaround_result ~= TURNAROUND_RESULT_ENUM.COLLECT) then --轮盘结果为非收集，则可以走动
                ...
                if not bonus_game_param.walk_end then
                    ...
                    for item_idx, special_item in ipairs(special_item_arr_config) do
                        if special_item.from_pos == curr_pos then
                            local special_item_use =
                                bonus_game_param.map_prize_use_info_arr.special_item_use_arr[item_idx]
                    ...
                    if use_special_item_idx then
                        ...
                        --奖励
                        local ordinary_item_use =
                            bonus_game_param.map_prize_use_info_arr.ordinary_item_use_arr[curr_pos]
                        local receive_prize, change_ordinary_item =
                            my_cal:ConsumMapPrizeWhenWalkIntoOridinary(
                            ordinary_item_arr_config[curr_pos],
                            ordinary_item_use,
    ...
    function SlotsNewPurrfectPetsSpin:BonusSettle()
        --取出变量
        ...
        if save_data.bonus_game_bouts > 0 then
            ...
            GameStatusCal.Calculate.UpdateGameStatus(player_game_status, 1, bonus_game_param.bonus_win) --Bonus这个流程完成
...
do --CollectBonus
    function SlotsNewPurrfectPetsSpin:CollectEnter()
        --取出变量
        local player = self.parameters.player
        local game_room_config = self.parameters.game_room_config
        local parameter = self.parameters.parameter
    ...
    function SlotsNewPurrfectPetsSpin:CollectOpenBox()
        --取出变量
        local session = self.parameters.session
        local task = session.task
        local player_game_status = self.parameters.player_game_status
        ...
        if (success) then
            --模拟一次BaseGame的Spin
            GameStatusCal.Calculate.AddBaseGameStatus(player_game_status, 1)
            ...
            if prize_type == COLLECT_PRIZE_ENUM.CHIP then --筹码类型奖励
                ...
                GameStatusCal.Calculate.UpdateGameStatus(player_game_status, 0, collect_win_chip) --将Bonus的钱放到Base中，直接结算
            elseif prize_type == COLLECT_PRIZE_ENUM.COIN_SPIN then --coin_spin类型
                ...
                my_cal:SetCoinSpinParamBeforeEnter(save_data, player, game_name, {}, true, 0, self.parameters)
            ...
            --添加BaseGame的次数，用来触发Bonus完成后的结算
            GameStatusCal.Calculate.UpdateGameStatus(player_game_status, 1, 0) --BaseGame次数+1
