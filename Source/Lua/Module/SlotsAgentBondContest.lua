module("SlotsAgentBondContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsAgentBondContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.AgentBond)
	then
		local designated_channel_id = request.player.agent_bond.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsAgentBondContainer:MakeDesignatedSit(SlotsAgentBondContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsAgentBondContainer:MakeSit(SlotsAgentBondContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsAgentBondContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsAgentBondContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsAgentBondContainer:MakeStand(player_id) then
		response.ret = Return.AGENT_BOND_CONTEST_NULL_EXIT()
		return response
	end
...
Offline = function ( _M, session, request )
	...
	local game_room_config = GameRoomConfig[GameType.AllTypes.AgentBond]
	local player_id = request.player_id
	LOG(RUN, INFO).Format("[SlotsAgentBondContest][Offline] player %s offline", player_id)
	local seat = SlotsAgentBondContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	...
	LOG(RUN, INFO).Format("[SlotsAgentBondContest][Offline] player %s offline table %s!", player_id, channel_id)
	return response
end
...
Slots = function ( _M, session, request )
	...
	local seat = SlotsAgentBondContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.AGENT_BOND_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end
	local player_id = request.player.id
	local step_num = request.player.agent_bond.step_num

	seat.player.agent_bond.step_num = step_num
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsAgentBondContest" then
		for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
				LOG(RUN, INFO).Format("[SlotsAgentBondContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
	end
	response.ret = Return.OK()
...
SlotsAgentBondContainer = {
	rooms = Container:Get("SlotsAgentBond.rooms"),
	player_seat = Container:Get("SlotsAgentBond.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
