module("SlotsMonkeyKingContest", package.seeall)
...
Offline = function ( _M, session, request )
	...
	local seat = SlotsMonkeyKingContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.MONKEYKING_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsMonkeyKingContainer:MakeStand(player_id) then
		response.ret = Return.MONKEYKING_CONTEST_NULL_EXIT()
		return response
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsMonkeyKingContest", seat.table.room.name, seat.table.id)
...
Hotfix = function(_M, session, request)
    ...
    if process_name == "SlotsMonkeyKingContest" then
        for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
                LOG(RUN, INFO).Format("[SlotsMonkeyKingContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
    end
...
SlotsMonkeyKingContainer = {
	rooms = Container:Get("SlotsMonkeyKing.rooms"),
	player_seat = Container:Get("SlotsMonkeyKing.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
