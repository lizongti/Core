module("SlotsChefsChoiceContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsChefsChoiceContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.ChefsChoice)
	then
		local designated_channel_id = request.player.chefs_choice.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsChefsChoiceContainer:MakeDesignatedSit(SlotsChefsChoiceContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsChefsChoiceContainer:MakeSit(SlotsChefsChoiceContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsChefsChoiceContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsChefsChoiceContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsChefsChoiceContainer:MakeStand(player_id) then
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsChefsChoiceContest" then
...
SlotsChefsChoiceContainer = {
	rooms = Container:Get("SlotsChefsChoice.rooms"),
	player_seat = Container:Get("SlotsChefsChoice.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
