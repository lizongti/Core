...
module("SlotsPurrfectPetsContest", package.seeall)
local GlobalSession = {}
Enter = function ( _M, session, request )
	...
	SlotsPurrfectPetsContainer:Init():NewRoom(game_room_config.room_name)
	if (request.player.game_type and request.player.game_type == GameType.AllTypes.PurrfectPets)
	then
		local designated_channel_id = request.player.purrfect_pets.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsPurrfectPetsContainer:MakeDesignatedSit(SlotsPurrfectPetsContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsPurrfectPetsContainer:MakeSit(SlotsPurrfectPetsContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsPurrfectPetsContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsPurrfectPetsContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsPurrfectPetsContainer:MakeStand(player_id) then
		response.ret = Return.PURRFECT_PETS_CONTEST_NULL_EXIT()
		return response
	end
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsPurrfectPetsContest" then
...
SlotsPurrfectPetsContainer = {
	rooms = Container:Get("SlotsPurrfectPets.rooms"),
	player_seat = Container:Get("SlotsPurrfectPets.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
