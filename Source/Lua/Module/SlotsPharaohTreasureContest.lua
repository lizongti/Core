module("SlotsPharaohTreasureContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsPharaohTreasureContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.PharaohTreasure)
	then
		local designated_channel_id = request.player.pharaoh_treasure.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsPharaohTreasureContainer:MakeDesignatedSit(SlotsPharaohTreasureContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsPharaohTreasureContainer:MakeSit(SlotsPharaohTreasureContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsPharaohTreasureContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsPharaohTreasureContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsPharaohTreasureContainer:MakeStand(player_id) then
		response.ret = Return.PHARAOHTREASURE_CONTEST_NULL_EXIT()
		return response
	end
...
Offline = function ( _M, session, request )
	...
	local seat = SlotsPharaohTreasureContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.PHARAOHTREASURE_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsPharaohTreasureContest" then
...
SlotsPharaohTreasureContainer = {
	rooms = Container:Get("SlotsPharaohTreasure.rooms"),
	player_seat = Container:Get("SlotsPharaohTreasure.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
