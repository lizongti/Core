...
--取出变量
local Types = _G["VampireRoseTypeArray"].Types
...
--方法对象初始化
SlotsVampireRoseSpin = {}

-- 入口
function SlotsVampireRoseSpin:Enter()
    --返回数据初始化
    ...
    if spin_status_info then
        if spin_status_info.status_id == GameStatusDefine.AllTypes.FreeSpinGame then
            curr_spin_type = SPIN_ENUM.FREE_SPIN
...
function SlotsVampireRoseSpin:NormalSpin()
    --取出变量
    ----统一取出
    local player = self.parameters.player
    local player_game_info = self.parameters.player_game_info
    ...
    local curr_base_total_win = GameStatusCal.Calculate.GetAllGameStatus(player_game_status)[1].total_win_chip
    --确定投注额度
    ...
    --统计信息
    --slots通用逻辑的特性字段
    special_parameter.win_chip_cal_multiple = 0
    --统计数据
    special_parameter.spin_type = save_data.curr_spin_type --当前spin类型
    special_parameter.win_jackpot = false --是否赢得了jackpot
    special_parameter.scatter_num = 0 --scatter的数量
    special_parameter.special_whole_col_num = 0 --整理为特殊图标的列数
    --赢钱的组成
    special_parameter.win_chip_form = {
        with_dynamic_wild = 0, --由于dynamic_wild带来的收益
        no_dynamic_wild = 0 --基础带来的收益
    }
    --free_spin的特殊统计
    special_parameter.trigger_free_spin = false --是否触发free_spin
    special_parameter.free_spin_multiple_times_when_settle = 0 --结算时free_spin结束时倍数
    ...
    local accumulated_win_chip = curr_base_total_win --之前累计赢的钱
    ...
    total_win_chip = total_win_chip + slots_win_chip --设置总赢钱
    ...
    --处理基础赢钱(中Jackpot奖励后，清除Jackpot的连线奖励)
        ...
        if slots_win_chip > 0 then
            ...
            for result_idx, prize_item_arr in pairs(all_prize_list) do
                ...
                if jackpot_prize_item then
                    ...
                    local win_chip_not_have_jackpot = slots_win_chip - jackpot_line_win --计算初始赢钱（不包含jackpot该连线）
                    --更新总赢钱
                    total_win_chip = win_chip_not_have_jackpot
                    formation_list[result_idx].slots_spin_list[1].win_chip = total_win_chip
                    --更新基础连线赢钱
                    slots_win_chip = win_chip_not_have_jackpot
                    formation_list[result_idx].slots_spin_list[1].slots_win_chip = slots_win_chip
                end
            end
        end
    end
    ...
    if need_handle_dynamic_wild then
        --变量声明
        local win_chip_origin = total_win_chip --起始金额
        ...
        if win_chip_origin ~= win_chip_final then
            table.insert(
                pre_action_list,
                {
                    ...
                    extra_win_chip = win_chip_final - win_chip_origin,
    ...
    if need_handle_jackpot then
        ...
        for result_idx, prize_item_arr in pairs(all_prize_list) do
            ...
            if jackpot_prize_item then
                ...
                --赢钱的处理
                local win_chip_origin = total_win_chip --初始赢钱
                local win_chip_final = total_win_chip --最终赢钱
                ...
                win_chip_final = win_chip_final + jackpot_line_win_curr --计算最终赢钱
                --更新总赢钱
                total_win_chip = win_chip_final
                formation_list[result_idx].slots_spin_list[1].win_chip = total_win_chip
                ...
                table.insert(
                    pre_action_list,
                    {
                        ...
                        extra_win_chip = jackpot_line_win_curr, --当前特性赢钱
    ...
    --设置用于大奖倍增的钱
    special_parameter.win_chip_cal_multiple = total_win_chip
    ...
    if need_free_spin_settle then
        ...
        --free_spin赢钱的更新
        free_spin_param.base_total_win_chip = free_spin_param.base_total_win_chip + total_win_chip --更新基础赢钱累计
        ...
        if (is_over) then
            --额外加倍的钱计算到总赢钱中
            local win_chip_origin = total_win_chip --初始赢钱
            local win_chip_final =
                total_win_chip + (free_spin_param.multiple_times - 1) * free_spin_param.base_total_win_chip --最终赢钱
            local extra_win_chip = win_chip_final - win_chip_origin

            --更新总赢钱
            total_win_chip = win_chip_final
            formation_list[1].slots_spin_list[1].win_chip = total_win_chip
...
function SlotsVampireRoseSpin:SpinBonusEnter()
    --取出变量
    local player_game_status = self.parameters.player_game_status
    local player = self.parameters.player
    local game_room_config = self.parameters.game_room_config
...
function SlotsVampireRoseSpin:SpinBonusSelect()
    --取出变量
    ...
    if spin_bonus_param.bouts > 0 then
        ...
        if slect_free_spin_bouts then --存在这个选项
            ...
            GameStatusCal.Calculate.UpdateGameStatus(player_game_status, 1, 0) --Bonus这个流程完成
            my_cal:SetFreeSpinBeforeEnter(
                save_data,
                select_param.total_amount,
                slect_free_spin_bouts,
                select_level,
                self.parameters
            ) --设置free_spin的参数
        end
