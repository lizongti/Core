module("SlotsChineseNewYearContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsChineseNewYearContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.ChineseNewYear)
	then
		local designated_channel_id = request.player.chinese_new_year.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsChineseNewYearContainer:MakeDesignatedSit(SlotsChineseNewYearContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsChineseNewYearContainer:MakeSit(SlotsChineseNewYearContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsChineseNewYearContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsChineseNewYearContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsChineseNewYearContainer:MakeStand(player_id) then
...
Offline = function ( _M, session, request )
	...
	LOG(RUN, INFO).Format("[SlotsChineseNewYearContest][Offline] player %s offline table %s!", player_id, channel_id)
	return response
end
...
Slots = function ( _M, session, request )
	...
	local step_max = #ChineseNewYearSlotsConfig
	local old_step_num = player.chinese_new_year.step_num
	player.chinese_new_year.step_num = player.chinese_new_year.step_num + step_num	
	player.chinese_new_year.step_num = (player.chinese_new_year.step_num - 1) % step_max + 1

	local slotsConfigInfo = ChineseNewYearSlotsConfig[step_num]
	...
	if (player.chinese_new_year.step_num < old_step_num)
	then
		winAmount = winAmount + amount * 5
	end

	response.step_num = player.chinese_new_year.step_num
	...
	local notice = {
		header = {
			...
			message_id = "SlotsChineseNewYearContest_Slots_Notice",
		},
		seat = {
			id = seat.id,
			online = seat.online,
		},
		step_num = player.chinese_new_year.step_num
	}
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsChineseNewYearContest" then
		for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
				LOG(RUN, INFO).Format("[SlotsChineseNewYearContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
	end
	response.ret = Return.OK()
...
SlotsChineseNewYearContainer = {
	rooms = Container:Get("SlotsChineseNewYear.rooms"),
	player_seat = Container:Get("SlotsChineseNewYear.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
