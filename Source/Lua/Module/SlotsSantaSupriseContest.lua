module("SlotsSantaSupriseContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsSantaSupriseContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.SantaSuprise)
	then
		local designated_channel_id = request.player.santa_suprise.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsSantaSupriseContainer:MakeDesignatedSit(SlotsSantaSupriseContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsSantaSupriseContainer:MakeSit(SlotsSantaSupriseContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsSantaSupriseContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsSantaSupriseContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsSantaSupriseContainer:MakeStand(player_id) then
		response.ret = Return.PIRATE_CONTEST_NULL_EXIT()
		return response
	end
...
Offline = function ( _M, session, request )
	...
	LOG(RUN, INFO).Format("[SlotsSantaSupriseContest][Offline] player %s offline table %s!", player_id, channel_id)
	return response
end
...
Slots = function ( _M, session, request )
	...
	local seat = SlotsSantaSupriseContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.PIRATE_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end
	...
	local step_num = math.rand_weight(nil, step_num_array)
	
	local step_max = #SantaSupriseSlotsConfig
	local old_step_num = player.santa_suprise.step_num
	player.santa_suprise.step_num = player.santa_suprise.step_num + step_num	
	player.santa_suprise.step_num = (player.santa_suprise.step_num - 1) % step_max + 1

	local slotsConfigInfo = SantaSupriseSlotsConfig[step_num]
	...
	if (player.santa_suprise.step_num < old_step_num)
	then
		winAmount = winAmount + amount * 5
	end

	response.step_num = player.santa_suprise.step_num
	...
	local notice = {
		header = {
			...
			message_id = "SlotsSantaSupriseContest_Slots_Notice",
		},
		seat = {
			id = seat.id,
			online = seat.online,
		},
		step_num = player.santa_suprise.step_num
	}
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsSantaSupriseContest" then
		for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
				LOG(RUN, INFO).Format("[SlotsSantaSupriseContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
	end
	response.ret = Return.OK()
...
SlotsSantaSupriseContainer = {
	rooms = Container:Get("SlotsSantaSuprise.rooms"),
	player_seat = Container:Get("SlotsSantaSuprise.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
