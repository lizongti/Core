require "Common/SlotsElvesEpicConstCal"
require "Common/LineNum"
module("SlotsElvesEpicContest", package.seeall)
local GlobalSession = {}
Enter = function ( _M, session, request )
	...
	--SlotsAliceinWonderlandContainer:MakeStand(player_id)
	
	local seat = nil
	local game_room_config = GameRoomConfig[GameType.AllTypes.ElvesEpic]

	SlotsElvesEpicContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.ElvesEpic)
	then
		local designated_channel_id = request.player.elves_epic.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsElvesEpicContainer:MakeDesignatedSit(SlotsElvesEpicContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsElvesEpicContainer:MakeSit(SlotsElvesEpicContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsElvesEpicContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsElvesEpicContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsElvesEpicContainer:MakeStand(player_id) then
		response.ret = Return.ELVESEPIC_CONTEST_NULL_EXIT()
		return response
	end
...
Award = function ( _M, session, request )
	...
	--local betArray = {2500, 5000, 12500, 25000, 50000, 125000, 250000, 500000, 1250000, 2500000, 5000000}
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsElvesEpicContest" then
		for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
				LOG(RUN, INFO).Format("[SlotsElvesEpicContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
	end
	response.ret = Return.OK()
...
SlotsElvesEpicContainer = {
	rooms = Container:Get("SlotsElvesEpic.rooms"),
	player_seat = Container:Get("SlotsElvesEpic.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
