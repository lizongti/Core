module("SlotsLuxuryLifeContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsLuxuryLifeContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.LuxuryLife)
	then
		local designated_channel_id = request.player.luxury_life.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsLuxuryLifeContainer:MakeDesignatedSit(SlotsLuxuryLifeContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsLuxuryLifeContainer:MakeSit(SlotsLuxuryLifeContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsLuxuryLifeContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsLuxuryLifeContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsLuxuryLifeContainer:MakeStand(player_id) then
...
Offline = function ( _M, session, request )
	...
	LOG(RUN, INFO).Format("[SlotsLuxuryLifeContest][Offline] player %s offline table %s!", player_id, channel_id)
	return response
end
...
Slots = function ( _M, session, request )
	...
	local step_max = #LuxuryLifeSlotsConfig
	local old_step_num = player.luxury_life.step_num
	player.luxury_life.step_num = player.luxury_life.step_num + step_num	
	player.luxury_life.step_num = (player.luxury_life.step_num - 1) % step_max + 1

	local slotsConfigInfo = LuxuryLifeSlotsConfig[step_num]
	...
	if (player.luxury_life.step_num < old_step_num)
	then
		winAmount = winAmount + amount * 5
	end

	response.step_num = player.luxury_life.step_num
	...
	local notice = {
		header = {
			...
			message_id = "SlotsLuxuryLifeContest_Slots_Notice",
		},
		seat = {
			id = seat.id,
			online = seat.online,
		},
		step_num = player.luxury_life.step_num
	}
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsLuxuryLifeContest" then
...
SlotsLuxuryLifeContainer = {
	rooms = Container:Get("SlotsLuxuryLife.rooms"),
	player_seat = Container:Get("SlotsLuxuryLife.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
