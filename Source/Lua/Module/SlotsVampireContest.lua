module("SlotsVampireContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsVampireContainer:Init():NewRoom(game_room_config.room_name)
	
	if (request.player.game_type and request.player.game_type == GameType.AllTypes.Vampire)
	then
		local designated_channel_id = request.player.vampire.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsVampireContainer:MakeDesignatedSit(SlotsVampireContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsVampireContainer:MakeSit(SlotsVampireContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsVampireContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsVampireContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsVampireContainer:MakeStand(player_id) then
		response.ret = Return.VAMPIRE_CONTEST_NULL_EXIT()
		return response
	end
...
Hotfix = function(_M, session, request)
    ...
    if process_name == "SlotsVampireContest" then
        for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
                LOG(RUN, INFO).Format("[SlotsVampireContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
    end
    response.ret = Return.OK()
...
SlotsVampireContainer = {
	rooms = Container:Get("SlotsVampire.rooms"),
	player_seat = Container:Get("SlotsVampire.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
