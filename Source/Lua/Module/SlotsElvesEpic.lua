require "Common/SlotsElvesEpicCal"
require "Common/RequestFilter"
require "Module/DailyTask"
require "Common/RobotAction"
require "Common/DailyMissionsCal"
module("SlotsElvesEpic", package.seeall)

-----------------------------------------------
-- 进入房间
------------------------------------------------
Enter = function(_M, session, request)
    local response = {header = {router = "Response"}}
    local filter_ret = RequestFilter.Filter("SlotsElvesEpic", "Enter", session, request, true)
    if filter_ret then
        response.ret = filter_ret
        return response
    end
    local task = session.task
    local player = session.player
    LOG(RUN, INFO).Format("[SlotsAliceinWonderlandContest][Enter]player %s", player.id)
    CommonCal.Calculate.MakeUpInRoom(session, task)
    local isLock = CommonCal.Calculate.LevelReq(player, GameType.AllTypes.ElvesEpic)
    if (isLock == 1) then
        response.ret = Return.LOCK_GAME()
        return response
    end

    local player_slots_info = CommonCal.Calculate.get_slots_info(session, task, player, GameType.AllTypes.ElvesEpic)
    local elves_epic = json.decode(player_slots_info.content)

    local table_id = request.table_id
    local async_request = nil
    local game_room_config = GameRoomConfig[GameType.AllTypes.ElvesEpic]

    if (table_id and table_id > 0) then
        local channel_id = string.format("%s.%s.%s", "SlotsElvesEpicContest", game_room_config.room_name, table_id)
        elves_epic.channel_id = channel_id
        async_request = {
            header = {
                router = "AsyncRequest",
                service_name = game_room_config.contest_client_name,
                task_id = task.id,
                module_id = "SlotsElvesEpicContest",
                message_id = "SlotsElvesEpicContest_Enter_Request"
            },
            player = {
                id = player.id,
                user = player.user,
                account = player.account,
                game_type = GameType.AllTypes.ElvesEpic,
                character = {
                    chip = player.character.chip,
                    vip = player.character.vip,
                    level = player.character.level,
                    experience = player.character.experience,
                    player_type = player.character.player_type
                },
                record = player.record,
                elves_epic = elves_epic
            }
        }
    else
        async_request = {
            header = {
                router = "AsyncRequest",
                service_name = game_room_config.contest_client_name,
                task_id = task.id,
                module_id = "SlotsElvesEpicContest",
                message_id = "SlotsElvesEpicContest_Enter_Request"
            },
            player = {
                id = player.id,
                user = player.user,
                account = player.account,
                character = {
                    chip = player.character.chip,
                    vip = player.character.vip,
                    level = player.character.level,
                    experience = player.character.experience,
                    player_type = player.character.player_type
                },
                record = player.record,
                elves_epic = elves_epic
            }
        }
    end

    local async_response = session:ContactPacket(task, async_request)
    if async_response.ret.code ~= 0 then
        response.ret = async_response.ret
        return response
    end
    local table_sync_notice = {
        header = {
            router = "Notice"
        },
        table = async_response.table
    }

    elves_epic.channel_id = async_response.channel_id

    --opt entertable
    local channel_id = async_response.channel_id
    local contest_id, room_id, table_id = unpack(string.split(channel_id, "."))
    local table_mates = {}
    for _, v in pairs(async_response.table.seat) do
        if v.player then
            table.insert(table_mates, v.player.id)
        end
    end

    Spark:EnterTable(
        player,
        {
            [1] = contest_id,
            [2] = room_id,
            [3] = table_id,
            [4] = #table_mates,
            [5] = table_mates
        }
    )

    local async_request = {
        header = {
            router = "AsyncRequest",
            service_name = "NotificationClientService",
            task_id = task.id,
            module_id = "Distributor",
            message_id = "Distributor_Register_Request"
        },
        session_id = session.id,
        player_id = player.id,
        channel_id = {async_response.channel_id},
        drop_channel_id = {"Hall"},
        player_type = session.player.character.player_type
    }
    local async_response = session:ContactPacket(task, async_request)
    if async_response.ret.code ~= 0 then
        response.ret = async_response.ret
        return response
    end

    async_request.header.service_name = game_room_config.contest_client_name
    local async_response = session:ContactPacket(task, async_request)
    if async_response.ret.code ~= 0 then
        response.ret = async_response.ret
        return response
    end

    elves_epic.bouts_id = 0
     --进场的时候把cd清空掉
    elves_epic.enter_chip = player.character.chip
    elves_epic.spined_times = 0

    -- response.item_ids = SlotsOpenSesameCal.Calculate.GenInitItem()
    response.ret = Return.OK()
    response.player = {
        elves_epic = elves_epic,
        character = {
            chip = player.character.chip
        }
    }

    player.game_type = GameType.AllTypes.ElvesEpic

    -----------------紧急修复-------------------
    if (elves_epic.free_spin_bouts > 50) then
        elves_epic.free_spin_bouts = 0
    end

    player_slots_info.content = json.encode(elves_epic)
    CommonCal.Calculate.UpdateSlotsToDbCache(task, player, player_slots_info)

    return response, table_sync_notice
end

UpdateBetAmount = function(_M, session, request)
    local response = {header = {router = "Response"}}

    local filter_ret = RequestFilter.Filter("SlotsElvesEpic", "UpdateBetAmount", session, request, true)
    if filter_ret then
        response.ret = filter_ret
        return response
    end

    local task = session.task

    local game_room_config = GameRoomConfig[GameType.AllTypes.ElvesEpic]
    local betAmount = request.amount
    local async_request = {
        header = {
            router = "AsyncRequest",
            service_name = game_room_config.contest_client_name,
            task_id = task.id,
            module_id = "SlotsElvesEpicContest",
            message_id = "SlotsElvesEpicContest_Award_Request"
        },
        amount = betAmount
    }

    local async_response = session:ContactPacket(task, async_request)

    response.ret = Return.OK()
    response.amount = async_response.amount

    return response
end

--开始slots
Start = function(_M, session, request)
    local response = {header = {router = "Response"}}

    local filter_ret = RequestFilter.Filter("SlotsElvesEpic", "Start", session, request, true)
    if filter_ret then
        response.ret = filter_ret
        return response
    end

    local LineNum = #SlotsElvesEpicCal.Const.Lines

    local is_get_free_spin = false
    local task = session.task
    local amount
    local player = session.player

    local player_slots_info = CommonCal.Calculate.get_slots_info(session, task, player, GameType.AllTypes.ElvesEpic)
    local elves_epic = json.decode(player_slots_info.content)

    CommonCal.Calculate.BeginStart(session, task, player)

    if GlobalState:IsSlotsMaintenance(session, task, player.id) == 1 then
        response.ret = Return.SERVER_NEAR_MAINENANCE()
        return response
    end

    if (player.game_type ~= GameType.AllTypes.ElvesEpic) then
        response.ret = Return.HAVE_ALREADY_EXIT_GAME()
        return response
    end
    local is_lock = CommonCal.Calculate.IsAppear(player)
    if (is_lock == 1) then
        response.ret = Return.LOCK_GAME()
        return response
    end
    local game_room_config = GameRoomConfig[GameType.AllTypes.ElvesEpic]
    local is_free_spin = elves_epic.free_spin_bouts > 0
    response.is_free_spin = is_free_spin and 1 or 0
    local origin_result, extra_wild, reel_file_name

    local all_prize_items = {}

    if (Base.Enviroment.pro_spec_t ~= "online" and player.character.player_type ~= tonumber(ConstValue[5].value)) then
        local async_request = {
            header = {
                router = "LocalRequest",
                service_name = "DispatcherService",
                task_id = task.id,
                module_id = "SlotsTest",
                message_id = "SlotsTest_Init_Request"
            },
            player_id = player.id
        }

        local async_response = session:ContactPacket(task, async_request)
    end

    local jackpotAmount = 0
    local chip_cost = 0
    --free spin不扣钱, free spin下amount不会改
    if is_free_spin then
        amount = elves_epic.bet_amount
        jackpotAmount = amount * LineNum
        elves_epic.free_spin_bouts = math.max(elves_epic.free_spin_bouts - 1, 0)
    else
        amount = request.amount
        chip_cost = amount * LineNum
        jackpotAmount = amount * LineNum

        if not Player:Consume(player, {"Chip", chip_cost}, Reason.ELVESEPIC_BET_CHIP_CONSUME()) then
            response.ret = Return.GAME_PLAYER_CHIP_NOT_ENOUGH()
            return response
        end
        elves_epic.bet_amount = amount
    end

    local flag = 0

    if (GlobalSlotsTest[player.id] ~= nil) then
        if (GlobalSlotsTest[player.id].flag == 1) then
            flag = 1
        end
    end

    local winAmount = 0
    local rsAmount = 0
    if (not winAmount) then
        winAmount = 0
    end

    local ElvesEpicBetAmountConfig = CommonCal.Calculate.get_config(player, "ElvesEpicBetAmountConfig")
    local bet_amount_conf
    for k, v in ipairs(ElvesEpicBetAmountConfig) do
        if v.single_amount == amount then
            bet_amount_conf = v
            break
        end
    end
    response.ret = Return.OK()

    local has_big_wild = 0
    if is_free_spin then
        origin_result, extra_wild, has_big_wild, reel_file_name =
            SlotsElvesEpicCal.Calculate.GenItemResult(player, true, winAmount, elves_epic.free_item_id)
    else
        origin_result, extra_wild, has_big_wild, reel_file_name =
            SlotsElvesEpicCal.Calculate.GenItemResult(player, false, winAmount, nil)
    end

    local result_with_wild = SlotsElvesEpicCal.Calculate.GenResultWithWild(origin_result, extra_wild)
    local prize_items, total_payrate =
        SlotsElvesEpicCal.Calculate.GenPrizeInfo(player, result_with_wild, winAmount, origin_result)
    table.insert(all_prize_items, prize_items)
    response.item_ids = SlotsElvesEpicCal.Calculate.TransResultToList(origin_result)
    response.prize_items = prize_items

    local free_spin_bouts, free_item_id = SlotsElvesEpicCal.Calculate.GenFreeSpinCount(result_with_wild)

    if (free_spin_bouts > 0) then
        is_get_free_spin = true
    end

    if (not is_free_spin and is_get_free_spin) then
        elves_epic.bouts_id = os.time()
    end

    if (not is_free_spin) then
        elves_epic.free_spin_bouts = elves_epic.free_spin_bouts + free_spin_bouts
    end

    if (not is_free_spin and elves_epic.free_spin_bouts <= 0) then --当不处于free状态并且没有free item时告诉客户端free_item_id为0
        free_item_id = 0
    end
    --[[
    if (elves_epic.free_spin_bouts <= 0 and is_free_spin)--当free spin由1为0时，处于is_free_spin,但为了客户端
    then
        free_item_id = 0
    end
    --]]
    --local jackpot_progress_delta = SlotsElvesEpicCal.Calculate.GenJackpotProgress(result_with_wild)
    --elves_epic.bonus_progress = elves_epic.bonus_progress + jackpot_progress_delta
    elves_epic.is_jackpot = 0

    elves_epic.free_item_id = free_item_id
    response.wilds = SlotsElvesEpicCal.Calculate.TransExtraWildPosToList(origin_result, has_big_wild)

    --if jackpot_progress_delta > 0 then
    --LOG(RUN, INFO).Format("[SlotsElvesEpic][Start] player %s's bonus progress increased %s", player.id, jackpot_progress_delta)
    --end

    ----记录record
    player.record.total_spin = player.record.total_spin + 1
    if is_free_spin then
        player.record.free_spin = player.record.free_spin + 1
    end

    if (winAmount > 0) then
        Player:Obtain(player, {"Chip", winAmount}, Reason.ELVESEPIC_BET_CHIP_JACKPOT())
    end
    local win_chip = total_payrate * amount
    if win_chip > 0 then
        Player:Obtain(player, {"Chip", win_chip}, Reason.ELVESEPIC_BET_CHIP_OBTAIN())

        --记录每日、每周赢钱,进排行榜
        RankHelper:ChallengeDailyWin(player)
        RankHelper:ChallengeWeeklyWin(player)
        --free spin不计入biggest win的统计
        local rep_free_spin = 0
        if (is_free_spin) then
            rep_free_spin = 1
        end
        --if not is_free_spin then
        local rep_data = {
            item_ids = response.item_ids,
            wilds = response.wilds,
            prize_items = prize_items,
            win_chip = win_chip,
            bet_amount = amount,
            free_spin = rep_free_spin
        }
        RankHelper:ChallengeDailyBiggestWin(player, GameType.AllTypes.ElvesEpic, rep_data)
        RankHelper:ChallengeWeeklyBiggestWin(player, GameType.AllTypes.ElvesEpic, rep_data)
        --end
        ----记录record
        player.record.spin_won = player.record.spin_won + 1
        player.record.total_win = player.record.total_win + win_chip
        if win_chip > player.record.biggest_win then
            player.record.biggest_win = win_chip
        end
    end

    if (has_big_wild > 0) then
        local number = win_chip / (amount * LineNum)
        number = math.floor(number + 0.5)

        Spark:FeatureSpecialWin(
            player,
            {
                [1] = GameType.AllTypes.ElvesEpic,
                [2] = "ElvesEpic",
                [3] = is_free_spin and 1 or 0,
                [4] = "WildFeature",
                [5] = number
            }
        )
    end

    -----记录free spin中总赢取
    if is_free_spin then
        elves_epic.free_total_win = elves_epic.free_total_win + win_chip
    else
        elves_epic.free_total_win = 0
    end

    DailyMissionsCal.Calculate.UpdateGameDailyMissions(
        session,
        player,
        amount,
        chip_cost,
        win_chip,
        is_free_spin,
        player.game_type,
        free_spin_bouts
    )

    local free_win_amount = 0
    if (is_free_spin) then
        free_win_amount = win_chip
    end

    local ElvesEpicPrizeConfig = CommonCal.Calculate.get_config(player, "ElvesEpicPrizeConfig")
    --accomplish tasks
    local task_req_data = {
        five_line = CommonCal.Calculate.GetFiveLineCount(all_prize_items),
        base_spin = not is_free_spin,
        free_spin = is_get_free_spin,
         --本次下注是否属于freespin
        win_amount = win_chip,
        bet_amount = amount * LineNum,
        max_bet = amount >= SlotsElvesEpicCal.Calculate.GetMaxBetAmount(player),
        free_win_amount = free_win_amount,
        epic_win = (total_payrate / LineNum) >= ElvesEpicPrizeConfig[3].min_multiple
    }
    DailyTask:CompleteTask(session, player, task_req_data)

    local history_games = json.decode(player.statistics.history_games)
    local is_exist = false
    for _, v in pairs(history_games) do
        if (v == player.game_type) then
            is_exist = true
            break
        end
    end
    if (not is_exist) then
        table.insert(history_games, player.game_type)
    end
    player.statistics.history_games = json.encode(history_games)
    player.statistics.last_game = player.game_type
    if ((win_chip / (amount * LineNum)) >= ElvesEpicPrizeConfig[3].min_multiple) then
        player.statistics.epicwin_num = player.statistics.epicwin_num + 1
    elseif ((win_chip / (amount * LineNum)) >= ElvesEpicPrizeConfig[2].min_multiple) then
        player.statistics.megawin_num = player.statistics.megawin_num + 1
    elseif ((win_chip / (amount * LineNum)) >= ElvesEpicPrizeConfig[1].min_multiple) then
        player.statistics.bigwin_num = player.statistics.bigwin_num + 1
    end

    Player:BroadCastChip(session, task, amount * LineNum, win_chip)

    local can_multiply = SlotsGameCal.Calculate.WinChipInMultiply(win_chip)
    if (can_multiply) then
        if ((total_payrate / LineNum) >= ElvesEpicPrizeConfig[1].min_multiple and not is_free_spin) then
            response.is_multiply = CommonCal.Calculate.IsMultiply(session, win_chip)
        end

        if ((total_payrate / LineNum) >= ElvesEpicPrizeConfig[1].min_multiple and not is_free_spin) then
            RobotAction.BigWinAction(session, task)
        end
    end

    --广播
    local win_info = {
        bet_amount = amount,
        total_bet = amount * LineNum,
        win_chip = win_chip
    }
    Communication:OnBcEvent(session, BroadcastType.AllTypes.ElvesEpicSpin, win_info, 15)

    --opt
    local contest_id, room_id, table_id = unpack(string.split(elves_epic.channel_id, "."))
    if (is_free_spin) then
        Spark:SlotsAward(
            player,
            {
                [1] = GameType.AllTypes.ElvesEpic,
                [2] = "ElvesEpic",
                [3] = table_id,
                [4] = elves_epic.bouts_id,
                [5] = elves_epic.bet_amount,
                [6] = win_chip,
                [7] = json.encode(origin_result),
                [8] = "[]",
                [9] = reel_file_name,
                [10] = elves_epic.free_spin_bouts,
                [11] = CommonCal.Calculate.GetFiveLineCount(all_prize_items) > 0 and 1 or 0,
                [12] = player.record.total_spin,
                [13] = amount * LineNum
            }
        )
    else
        Spark:SlotsStart(
            player,
            {
                [1] = GameType.AllTypes.ElvesEpic,
                [2] = "ElvesEpic",
                [3] = table_id,
                [4] = elves_epic.bouts_id,
                [5] = amount,
                [6] = amount * LineNum,
                [7] = win_chip,
                [8] = json.encode(origin_result),
                [9] = "[]",
                [10] = is_free_spin,
                [11] = reel_file_name,
                [12] = CommonCal.Calculate.GetFiveLineCount(all_prize_items) > 0 and 1 or 0,
                [13] = player.record.total_spin
            }
        )
    end

    --buyloss的局数+1
    elves_epic.jackpot_win_chip = winAmount
    elves_epic.spined_times = elves_epic.spined_times + 1
    response.amount = rsAmount
    response.win_chip = win_chip

    CommonCal.Calculate.EndStart(session, task, player, request, response, elves_epic, LineNum, chip_cost, win_chip)

    --gain exp
    if not is_free_spin then
        local exp = chip_cost
        local exp_request = {
            type = "SlotsElvesEpic",
            ante_gold = amount * LineNum,
            gain_exp = exp
        }
        Player:GainExp(session, exp_request)
    end

    player_slots_info.content = json.encode(elves_epic)
    CommonCal.Calculate.UpdateSlotsToDbCache(task, player, player_slots_info)

    --锦标赛玩家得分更新
    CommonCal.Calculate.UpdateTournamentPlayerInfo(session, player.game_type, player, chip_cost, win_chip)

    response.player = {
        unlock_games = player.unlock_games,
        character = {
            chip = player.character.chip,
            experience = player.character.experience,
            level = player.character.level
        },
        elves_epic = {
            bet_amount = amount,
            bonus_progress = elves_epic.bonus_progress,
            free_spin_bouts = elves_epic.free_spin_bouts,
            free_total_win = elves_epic.free_total_win,
            is_jackpot = elves_epic.is_jackpot,
            free_item_id = elves_epic.free_item_id
        }
    }

    return response
end

-----------------------------------------------
-- 退出房间
------------------------------------------------
Exit = function(_M, session, request)
    local response = {header = {router = "Response"}}
    local filter_ret = RequestFilter.Filter("SlotsElvesEpic", "Exit", session, request, true)
    if filter_ret then
        response.ret = filter_ret
        return response
    end

    local player = session.player
    local task = session.task

    local player_slots_info = CommonCal.Calculate.get_slots_info(session, task, player, GameType.AllTypes.ElvesEpic)
    local elves_epic = json.decode(player_slots_info.content)

    local game_room_config = GameRoomConfig[player.game_type]
    if (game_room_config == nil) then
        response.ret = Return.HAVE_ALREADY_EXIT_GAME()
        return response
    end
    local async_request = {
        header = {
            router = "AsyncRequest",
            service_name = game_room_config.contest_client_name,
            task_id = task.id,
            module_id = "SlotsElvesEpicContest",
            message_id = "SlotsElvesEpicContest_Exit_Request"
        },
        player_id = player.id
    }
    local async_response = session:ContactPacket(task, async_request)
    if async_response.ret.code ~= 0 then
        response.ret = async_response.ret
        return response
    end

    -- opt exit
    local contest_id, room_id, table_id = unpack(string.split(async_response.channel_id, "."))
    local table_mates = {}
    for _, v in pairs(async_response.table.seat) do
        if v.player then
            table.insert(table_mates, v.player.id)
        end
    end

    Spark:LeaveTable(
        player,
        {
            [1] = contest_id,
            [2] = room_id,
            [3] = table_id,
            [4] = #table_mates,
            [5] = table_mates
        }
    )

    local async_request = {
        header = {
            router = "AsyncRequest",
            service_name = "NotificationClientService",
            task_id = task.id,
            module_id = "Distributor",
            message_id = "Distributor_Register_Request"
        },
        session_id = session.id,
        player_id = player.id,
        channel_id = {"Hall"},
        drop_channel_id = {async_response.channel_id},
        player_type = session.player.character.player_type
    }
    local async_response = session:ContactPacket(task, async_request)
    if async_response.ret.code ~= 0 then
        response.ret.code = async_response.ret.code
        return response
    end

    async_request.header.service_name = game_room_config.contest_client_name
    local async_response = session:ContactPacket(task, async_request)
    if async_response.ret.code ~= 0 then
        response.ret = async_response.ret
        return response
    end

    local trigger_buyloss, total_loss, diamond, goods_id =
        BuyLoss:Trigger(session, task, GameType.AllTypes.ElvesEpic, player) ---当玩家钱减少到一定数量时，提示玩家充值
    if trigger_buyloss then
        elves_epic.total_loss = total_loss
        session:WriteRouterPacket(
            {
                header = {
                    router = "SpecificNotice",
                    session_id = session.id,
                    player_id = player.id,
                    module_id = "BuyLoss",
                    message_id = "BuyLoss_Trigger_Notice"
                },
                total_loss = total_loss,
                diamond = diamond,
                goods_id = goods_id
            }
        )
    end
    elves_epic.spined_times = 0

    response = {
        header = response.header,
        ret = Return.OK(),
        player = {
            character = {
                chip = player.character.chip
            }
        }
    }

    player_slots_info.content = json.encode(elves_epic)
    CommonCal.Calculate.UpdateSlotsToDbCache(task, player, player_slots_info)

    LOG(RUN, INFO).Format("[SlotsElvesEpic][Exit] ok player %s", player.id)
    return response
end
