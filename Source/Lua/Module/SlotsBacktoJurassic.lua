require "Common/SlotsBacktoJurassicCal"
require "Common/RequestFilter"
require "Module/DailyTask"
require "Common/CommonCal"
require "Common/RobotAction"
module("SlotsBacktoJurassic", package.seeall)

-----------------------------------------------
-- 进入房间
------------------------------------------------
Enter = function(_M, session, request)
    local response = {header = {router = "Response"}}

    local filter_ret = RequestFilter.Filter("SlotsBacktoJurassic", "Enter", session, request, true)
    if filter_ret then
        response.ret = filter_ret
        return response
    end

    local task = session.task
    local player = session.player

    local player_slots_info =
        CommonCal.Calculate.get_slots_info(session, task, player, GameType.AllTypes.BacktoJurassic)
    local backto_jurassic = json.decode(player_slots_info.content)

    CommonCal.Calculate.MakeUpInRoom(session, task)

    LOG(RUN, INFO).Format("[SlotsBacktoJurassic][Enter] player %s", player.id)

    local isLock = CommonCal.Calculate.LevelReq(player, GameType.AllTypes.BacktoJurassic)
    if (isLock == 1) then
        response.ret = Return.LOCK_GAME()
        return response
    end

    local table_id = request.table_id
    local async_request = nil
    local game_room_config = GameRoomConfig[GameType.AllTypes.BacktoJurassic]
    if (table_id and table_id > 0) then
        local channel_id = string.format("%s.%s.%s", "SlotsBacktoJurassicContest", game_room_config.room_name, table_id)
        backto_jurassic.channel_id = channel_id
        async_request = {
            header = {
                router = "AsyncRequest",
                service_name = game_room_config.contest_client_name,
                task_id = task.id,
                module_id = "SlotsBacktoJurassicContest",
                message_id = "SlotsBacktoJurassicContest_Enter_Request"
            },
            player = {
                id = player.id,
                user = player.user,
                account = player.account,
                game_type = GameType.AllTypes.BacktoJurassic,
                character = {
                    chip = player.character.chip,
                    vip = player.character.vip,
                    level = player.character.level,
                    experience = player.character.experience,
                    player_type = player.character.player_type
                },
                record = player.record,
                backto_jurassic = backto_jurassic
            }
        }
    else
        async_request = {
            header = {
                router = "AsyncRequest",
                service_name = game_room_config.contest_client_name,
                task_id = task.id,
                module_id = "SlotsBacktoJurassicContest",
                message_id = "SlotsBacktoJurassicContest_Enter_Request"
            },
            player = {
                id = player.id,
                user = player.user,
                account = player.account,
                character = {
                    chip = player.character.chip,
                    vip = player.character.vip,
                    level = player.character.level,
                    experience = player.character.experience,
                    player_type = player.character.player_type
                },
                record = player.record,
                backto_jurassic = backto_jurassic
            }
        }
    end

    local async_response = session:ContactPacket(task, async_request)

    if async_response.ret.code ~= 0 then
        response.ret = async_response.ret
        return response
    end
    local table_sync_notice = {
        header = {
            router = "Notice"
        },
        table = async_response.table
    }

    backto_jurassic.channel_id = async_response.channel_id

    --opt entertable
    local channel_id = async_response.channel_id
    local contest_id, room_id, table_id = unpack(string.split(channel_id, "."))
    local table_mates = {}
    for _, v in pairs(async_response.table.seat) do
        if v.player then
            table.insert(table_mates, v.player.id)
        end
    end

    Spark:EnterTable(
        player,
        {
            [1] = contest_id,
            [2] = room_id,
            [3] = table_id,
            [4] = #table_mates,
            [5] = table_mates
        }
    )

    local async_request = {
        header = {
            router = "AsyncRequest",
            service_name = "NotificationClientService",
            task_id = task.id,
            module_id = "Distributor",
            message_id = "Distributor_Register_Request"
        },
        session_id = session.id,
        player_id = player.id,
        channel_id = {async_response.channel_id},
        drop_channel_id = {"Hall"},
        player_type = session.player.character.player_type
    }

    local async_response = session:ContactPacket(task, async_request)

    if async_response.ret.code ~= 0 then
        response.ret = async_response.ret
        return response
    end

    async_request.header.service_name = game_room_config.contest_client_name

    local async_response = session:ContactPacket(task, async_request)

    if async_response.ret.code ~= 0 then
        response.ret = async_response.ret
        return response
    end

    backto_jurassic.bouts_id = 0
     --进场的时候把cd清空掉
    backto_jurassic.enter_chip = player.character.chip
    backto_jurassic.spined_times = 0

    if (backto_jurassic.free_spin_num_str) then
        local free_spin_num_array = json.decode(backto_jurassic.free_spin_num_str)
        SlotsBacktoJurassicCal.Calculate.UpdateFreeBous(backto_jurassic, free_spin_num_array)

        if (free_spin_num_array and #free_spin_num_array > 0) then
        --free_spin_type = tonumber(free_spin_num_array[1].free_spin_type)
        end
    end

    if backto_jurassic.free_spin_type == 0 then
        backto_jurassic.free_spin_type = 2
    end

    response.ret = Return.OK()
    response.player = {
        backto_jurassic = backto_jurassic,
        character = {
            chip = player.character.chip
        },
        backto_jurassic = backto_jurassic
    }
    player.game_type = GameType.AllTypes.BacktoJurassic

    -----------------紧急修复-------------------
    player_slots_info.content = json.encode(backto_jurassic)
    CommonCal.Calculate.UpdateSlotsToDbCache(task, player, player_slots_info)

    return response, table_sync_notice
end

--开始slots
Start = function(_M, session, request)
    local response = {header = {router = "Response"}}

    local filter_ret = RequestFilter.Filter("SlotsBacktoJurassic", "Start", session, request, true)

    if filter_ret then
        response.ret = filter_ret
        return response
    end

    local LineNum = #SlotsBacktoJurassicCal.Const.Lines

    local task = session.task
    local amount
    local player = session.player
    CommonCal.Calculate.BeginStart(session, task, player)

    if GlobalState:IsSlotsMaintenance(session, task, player.id) == 1 then
        response.ret = Return.SERVER_NEAR_MAINENANCE()
        return response
    end

    if (player.game_type ~= GameType.AllTypes.BacktoJurassic) then
        response.ret = Return.HAVE_ALREADY_EXIT_GAME()
        return response
    end
    local is_lock = CommonCal.Calculate.IsAppear(player)
    if (is_lock == 1) then
        response.ret = Return.LOCK_GAME()
        return response
    end

    local player_slots_info =
        CommonCal.Calculate.get_slots_info(session, task, player, GameType.AllTypes.BacktoJurassic)
    local backto_jurassic = json.decode(player_slots_info.content)

    local all_prize_items = {}

    local free_spin_num_array = {}
    --free spin是上一次记录的
    if (backto_jurassic.free_spin_num_str) then
        free_spin_num_array = json.decode(backto_jurassic.free_spin_num_str)
        SlotsBacktoJurassicCal.Calculate.UpdateFreeBous(backto_jurassic, free_spin_num_array)
    end

    local is_free_spin = backto_jurassic.free_spin_bouts > 0

    response.is_free_spin = is_free_spin and 1 or 0
    local is_get_free_spin = false

    if (Base.Enviroment.pro_spec_t ~= "online" and player.character.player_type ~= tonumber(ConstValue[5].value)) then
        local async_request = {
            header = {
                router = "LocalRequest",
                service_name = "DispatcherService",
                task_id = task.id,
                module_id = "SlotsTest",
                message_id = "SlotsTest_Init_Request"
            },
            player_id = player.id
        }

        local async_response = session:ContactPacket(task, async_request)
    end
    --free spin不扣钱, free spin下amount不会改
    local chip_cost = 0
    if is_free_spin then
        amount = backto_jurassic.bet_amount

        if (#free_spin_num_array > 0) then
            if (free_spin_num_array[1].free_spin_bouts == 0) then
                table.remove(free_spin_num_array, 1)
            end

            free_spin_num_array[1].free_spin_bouts = math.max(tonumber(free_spin_num_array[1].free_spin_bouts) - 1, 0)
        end

        SlotsBacktoJurassicCal.Calculate.UpdateFreeBous(backto_jurassic, free_spin_num_array)
    else
        amount = request.amount
        chip_cost = amount * LineNum

        if not Player:Consume(player, {"Chip", chip_cost}, Reason.BACKTO_JURASSIC_BET_CHIP_CONSUME()) then
            response.ret = Return.GAME_PLAYER_CHIP_NOT_ENOUGH()
            return response
        end
        backto_jurassic.bet_amount = amount
        free_spin_num_array = {}
    end

    local BacktoJurassicBetAmountConfig = CommonCal.Calculate.get_config(player, "BacktoJurassicBetAmountConfig")
    local bet_amount_conf
    for k, v in ipairs(BacktoJurassicBetAmountConfig) do
        if v.single_amount == amount then
            bet_amount_conf = v
            break
        end
    end

    local free_spin_type = 0
    if (#free_spin_num_array > 0) then
        free_spin_type = tonumber(free_spin_num_array[1].free_spin_type)
    end

    local origin_result, extra_wild, reel_file_name
    local item_list = {}

    if is_free_spin then
        origin_result, reel_file_name = SlotsBacktoJurassicCal.Calculate.GenItemResult(player, free_spin_type)
    else
        origin_result, reel_file_name = SlotsBacktoJurassicCal.Calculate.GenItemResult(player, nil)
    end

    local local_big_result = table.DeepCopy(origin_result)
    SlotsBacktoJurassicCal.Calculate.ReplaceBigItem(local_big_result)
    local item_ids = SlotsBacktoJurassicCal.Calculate.TransResultToList(local_big_result)

    local prize_items, total_payrate =
        SlotsBacktoJurassicCal.Calculate.GenPrizeInfo(player, origin_result, free_spin_type)
    table.insert(all_prize_items, prize_items)
    local win_chip = total_payrate * amount

    local free_spin_bouts, free_item_id = SlotsBacktoJurassicCal.Calculate.GenFreeSpinCount(origin_result)

    if (free_spin_bouts > 0) then
        is_get_free_spin = true

        local local_weight_tab = {[2] = 0.1, [3] = 0.1, [4] = 0.1, [5] = 0.1, [6] = 0.1}
        local local_free_spin_type = math.rand_weight(player, local_weight_tab)

        local free_spin_item = {}
        free_spin_item["free_spin_type"] = local_free_spin_type
        free_spin_item["free_spin_bouts"] = 10
        table.insert(free_spin_num_array, free_spin_item)
    end

    if (not is_free_spin and is_get_free_spin) then
        backto_jurassic.bouts_id = os.time()
    end

    if (not is_free_spin and backto_jurassic.free_spin_bouts <= 0) then --当不处于free状态并且没有free item时告诉客户端free_item_id为0
        free_item_id = 0
    end
    backto_jurassic.free_item_id = free_item_id

    ----记录record
    player.record.total_spin = player.record.total_spin + 1
    if is_free_spin then
        player.record.free_spin = player.record.free_spin + 1
    end

    -----记录free spin中总赢取

    if is_free_spin then
        backto_jurassic.free_total_win = backto_jurassic.free_total_win + win_chip
    else
        backto_jurassic.free_total_win = 0
    end

    DailyMissionsCal.Calculate.UpdateGameDailyMissions(
        session,
        player,
        amount,
        chip_cost,
        win_chip,
        is_free_spin,
        player.game_type,
        free_spin_bouts
    )

    local free_win_amount = 0
    if (is_free_spin) then
        free_win_amount = win_chip
    end

    local BacktoJurassicPrizeConfig = CommonCal.Calculate.get_config(player, "BacktoJurassicPrizeConfig")

    local task_req_data = {
        five_line = CommonCal.Calculate.GetFiveLineCount(all_prize_items),
        base_spin = not is_free_spin,
        free_spin = free_spin_bouts > 0,
         --本次下注是否触发freespin
        win_amount = win_chip,
        bet_amount = amount * LineNum,
        max_bet = amount >= SlotsBacktoJurassicCal.Calculate.GetMaxBetAmount(player),
        free_win_amount = free_win_amount,
        epic_win = (total_payrate / LineNum) >= BacktoJurassicPrizeConfig[3].min_multiple
    }
    DailyTask:CompleteTask(session, player, task_req_data)

    local history_games = json.decode(player.statistics.history_games)
    local is_exist = false
    for _, v in pairs(history_games) do
        if (v == player.game_type) then
            is_exist = true
            break
        end
    end
    if (not is_exist) then
        table.insert(history_games, player.game_type)
    end
    player.statistics.history_games = json.encode(history_games)
    player.statistics.last_game = player.game_type
    if ((win_chip / (amount * LineNum)) >= BacktoJurassicPrizeConfig[3].min_multiple) then
        player.statistics.epicwin_num = player.statistics.epicwin_num + 1
    elseif ((win_chip / (amount * LineNum)) >= BacktoJurassicPrizeConfig[2].min_multiple) then
        player.statistics.megawin_num = player.statistics.megawin_num + 1
    elseif ((win_chip / (amount * LineNum)) >= BacktoJurassicPrizeConfig[1].min_multiple) then
        player.statistics.bigwin_num = player.statistics.bigwin_num + 1
    end

    Player:BroadCastChip(session, task, amount * LineNum, win_chip)

    local can_multiply = SlotsGameCal.Calculate.WinChipInMultiply(win_chip)
    if (can_multiply) then
        if ((total_payrate / LineNum) >= BacktoJurassicPrizeConfig[1].min_multiple and not is_free_spin) then
            response.is_multiply = CommonCal.Calculate.IsMultiply(session, win_chip)
        end

        if ((total_payrate / LineNum) >= BacktoJurassicPrizeConfig[1].min_multiple and not is_free_spin) then
            RobotAction.BigWinAction(session, task)
        end
    end

    --广播
    local win_info = {
        bet_amount = amount,
        total_bet = amount * LineNum,
        win_chip = win_chip
    }
    Communication:OnBcEvent(session, BroadcastType.AllTypes.BacktoJurassic, win_info, 15)

    if win_chip > 0 then
        Player:Obtain(player, {"Chip", win_chip}, Reason.BACKTO_JURASSIC_BET_CHIP_OBTAIN())

        --记录每日、每周赢钱,进排行榜
        RankHelper:ChallengeDailyWin(player)
        RankHelper:ChallengeWeeklyWin(player)
        --free spin不计入biggest win的统计
        --if not is_free_spin then
        local rep_free_spin = 0
        if (is_free_spin) then
            rep_free_spin = 1
        end
        local rep_data = {
            item_ids = item_ids,
            prize_items = prize_items,
            win_chip = win_chip,
            bet_amount = amount,
            free_spin = rep_free_spin
        }
        RankHelper:ChallengeDailyBiggestWin(player, GameType.AllTypes.BacktoJurassic, rep_data)
        RankHelper:ChallengeWeeklyBiggestWin(player, GameType.AllTypes.BacktoJurassic, rep_data)
        --end
        ----记录record
        player.record.spin_won = player.record.spin_won + 1
        player.record.total_win = player.record.total_win + win_chip
        if win_chip > player.record.biggest_win then
            player.record.biggest_win = win_chip
        end
    end

    --buyloss的局数+1
    backto_jurassic.spined_times = backto_jurassic.spined_times + 1

    backto_jurassic.free_spin_num_str = json.encode(free_spin_num_array)
    SlotsBacktoJurassicCal.Calculate.UpdateFreeBous(backto_jurassic, free_spin_num_array)

    response.item_ids = item_ids
    response.prize_items = prize_items
    response.win_chip = win_chip

    --这里记录玩家free spin的类型
    --backto_jurassic.free_spin_type = 0--free_spin_type

    if (free_spin_num_array and #free_spin_num_array > 0) then
        free_spin_type = tonumber(free_spin_num_array[1].free_spin_type)
    --backto_jurassic.free_spin_type = 0--free_spin_type
    end

    local contest_id, room_id, table_id = unpack(string.split(backto_jurassic.channel_id, "."))

    if (is_free_spin) then
        Spark:SlotsAward(
            player,
            {
                [1] = GameType.AllTypes.BacktoJurassic,
                [2] = "BacktoJurassic",
                [3] = table_id,
                [4] = backto_jurassic.bouts_id,
                [5] = backto_jurassic.bet_amount,
                [6] = win_chip,
                [7] = json.encode(origin_result),
                [8] = "[]",
                [9] = reel_file_name,
                [10] = backto_jurassic.free_spin_bouts,
                [11] = CommonCal.Calculate.GetFiveLineCount(all_prize_items) > 0 and 1 or 0,
                [12] = player.record.total_spin,
                [13] = amount * LineNum
            }
        )
    else
        Spark:SlotsStart(
            player,
            {
                [1] = GameType.AllTypes.BacktoJurassic,
                [2] = "BacktoJurassic",
                [3] = table_id,
                [4] = backto_jurassic.bouts_id,
                [5] = amount,
                [6] = amount * LineNum,
                [7] = win_chip,
                [8] = json.encode(origin_result),
                [9] = "[]",
                [10] = is_free_spin,
                [11] = reel_file_name,
                [12] = CommonCal.Calculate.GetFiveLineCount(all_prize_items) > 0 and 1 or 0,
                [13] = player.record.total_spin
            }
        )
    end

    CommonCal.Calculate.EndStart(
        session,
        task,
        player,
        request,
        response,
        backto_jurassic,
        LineNum,
        chip_cost,
        win_chip
    )

    --gain exp
    if not is_free_spin then
        local exp = chip_cost
        local exp_request = {
            type = "SlotsBacktoJurassic",
            ante_gold = amount * LineNum,
            gain_exp = exp
        }
        Player:GainExp(session, exp_request)
    end

    player_slots_info.content = json.encode(backto_jurassic)
    CommonCal.Calculate.UpdateSlotsToDbCache(task, player, player_slots_info)

    --锦标赛玩家得分更新
    CommonCal.Calculate.UpdateTournamentPlayerInfo(session, player.game_type, player, chip_cost, win_chip)

    response.ret = Return.OK()

    response.player = {
        unlock_games = player.unlock_games,
        character = {
            chip = player.character.chip,
            experience = player.character.experience,
            level = player.character.level
        },
        backto_jurassic = {
            bet_amount = amount,
            bonus_progress = backto_jurassic.bonus_progress,
            free_spin_bouts = backto_jurassic.free_spin_bouts,
            free_total_win = backto_jurassic.free_total_win,
            free_item_id = backto_jurassic.free_item_id,
            gift_item = backto_jurassic.gift_item,
            free_spin_type = backto_jurassic.free_spin_type
        }
    }

    return response
end

-----------------------------------------------
-- 退出房间
------------------------------------------------
Exit = function(_M, session, request)
    local response = {header = {router = "Response"}}

    local filter_ret = RequestFilter.Filter("SlotsBacktoJurassic", "Exit", session, request, true)
    if filter_ret then
        response.ret = filter_ret
        return response
    end

    local player = session.player
    local task = session.task
    local game_room_config = GameRoomConfig[player.game_type]
    if (game_room_config == nil) then
        response.ret = Return.HAVE_ALREADY_EXIT_GAME()
        return response
    end
    local player_slots_info =
        CommonCal.Calculate.get_slots_info(session, task, player, GameType.AllTypes.BacktoJurassic)
    local backto_jurassic = json.decode(player_slots_info.content)

    local async_request = {
        header = {
            router = "AsyncRequest",
            service_name = game_room_config.contest_client_name,
            task_id = task.id,
            module_id = "SlotsBacktoJurassicContest",
            message_id = "SlotsBacktoJurassicContest_Exit_Request"
        },
        player_id = player.id
    }

    local async_response = session:ContactPacket(task, async_request)

    if async_response.ret.code ~= 0 then
        response.ret = async_response.ret
        return response
    end

    -- opt exit
    local contest_id, room_id, table_id = unpack(string.split(async_response.channel_id, "."))
    local table_mates = {}
    for _, v in pairs(async_response.table.seat) do
        if v.player then
            table.insert(table_mates, v.player.id)
        end
    end

    Spark:LeaveTable(
        player,
        {
            [1] = contest_id,
            [2] = room_id,
            [3] = table_id,
            [4] = #table_mates,
            [5] = table_mates
        }
    )

    local async_request = {
        header = {
            router = "AsyncRequest",
            service_name = "NotificationClientService",
            task_id = task.id,
            module_id = "Distributor",
            message_id = "Distributor_Register_Request"
        },
        session_id = session.id,
        player_id = player.id,
        channel_id = {"Hall"},
        drop_channel_id = {async_response.channel_id},
        player_type = session.player.character.player_type
    }
    local async_response = session:ContactPacket(task, async_request)
    if async_response.ret.code ~= 0 then
        response.ret.code = async_response.ret.code
        return response
    end

    async_request.header.service_name = game_room_config.contest_client_name
    local async_response = session:ContactPacket(task, async_request)
    if async_response.ret.code ~= 0 then
        response.ret = async_response.ret
        return response
    end

    local trigger_buyloss, total_loss, diamond, goods_id =
        BuyLoss:Trigger(session, task, GameType.AllTypes.BacktoJurassic, player) ---当玩家钱减少到一定数量时，提示玩家充值
    if trigger_buyloss then
        backto_jurassic.total_loss = total_loss
        session:WriteRouterPacket(
            {
                header = {
                    router = "SpecificNotice",
                    session_id = session.id,
                    player_id = player.id,
                    module_id = "BuyLoss",
                    message_id = "BuyLoss_Trigger_Notice"
                },
                total_loss = total_loss,
                diamond = diamond,
                goods_id = goods_id
            }
        )
    end
    backto_jurassic.spined_times = 0
    response = {
        header = response.header,
        ret = Return.OK(),
        player = {
            character = {
                chip = player.character.chip
            }
        }
    }

    player_slots_info.content = json.encode(backto_jurassic)
    CommonCal.Calculate.UpdateSlotsToDbCache(task, player, player_slots_info)

    LOG(RUN, INFO).Format("[SlotsBacktoJurassic][Exit] ok player %s", player.id)
    return response
end

SelectFreeSpinType = function(_M, session, request)
    local response = {header = {router = "Response"}}

    local filter_ret = RequestFilter.Filter("SlotsBacktoJurassic", "Exit", session, request, true)
    if filter_ret then
        response.ret = filter_ret
        return response
    end

    local player = session.player

    local player_slots_info =
        CommonCal.Calculate.get_slots_info(session, task, player, GameType.AllTypes.BacktoJurassic)
    local backto_jurassic = json.decode(player_slots_info.content)

    backto_jurassic.free_spin_type = request.free_spin_type

    response.ret = Return.OK()
    response.player = {
        backto_jurassic = {
            free_spin_type = request.free_spin_type
        }
    }

    player_slots_info.content = json.encode(backto_jurassic)
    CommonCal.Calculate.UpdateSlotsToDbCache(task, player, player_slots_info)

    return response
end
