module("SlotsDragonTaleContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsDragonTaleContainer:Init():NewRoom(game_room_config.room_name)
	
	if (request.player.game_type and request.player.game_type == GameType.AllTypes.DragonTale)
	then
		local designated_channel_id = request.player.dragon_tale.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsDragonTaleContainer:MakeDesignatedSit(SlotsDragonTaleContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsDragonTaleContainer:MakeSit(SlotsDragonTaleContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsDragonTaleContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsDragonTaleContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsDragonTaleContainer:MakeStand(player_id) then
...
-- 	for room_id, room in pairs(SlotsDragonTaleContainer.rooms) do
...
--                     if SlotsDragonTaleContainer:MakeStand(player_id) then
--                         local notice = {
--                             header = {
--                                 router = "ContestBroadcast",
--                                 channel_id = channel_id,
...
Hotfix = function(_M, session, request)
    ...
    if process_name == "SlotsDragonTaleContest" then
...
SlotsDragonTaleContainer = {
	rooms = Container:Get("SlotsDragonTale.rooms"),
	player_seat = Container:Get("SlotsDragonTale.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
