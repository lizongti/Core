module("SlotsBacktoJurassicContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsBacktoJurassicContainer:Init():NewRoom(game_room_config.room_name)

	local seat = nil
	if (request.player.game_type and request.player.game_type == GameType.AllTypes.BacktoJurassic)
	then
		local designated_channel_id = request.player.backto_jurassic.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsBacktoJurassicContainer:MakeDesignatedSit(SlotsBacktoJurassicContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsBacktoJurassicContainer:MakeSit(SlotsBacktoJurassicContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsBacktoJurassicContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsBacktoJurassicContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsBacktoJurassicContainer:MakeStand(player_id) then
		response.ret = Return.BACKTO_JURASSIC_CONTEST_NULL_EXIT()
		return response
	end
...
Offline = function ( _M, session, request )
	...
	LOG(RUN, INFO).Format("[SlotsBacktoJurassicContest][Offline] player %s offline table %s!", player_id, channel_id)
	return response
end
...
Slots = function ( _M, session, request )
	...
	local seat = SlotsBacktoJurassicContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.CANNOT_FIND_PLAYER()
		return response
	end
	...
	local step_max = #BacktoJurassicSlotsConfig
	local old_step_num = player.backto_jurassic.step_num
	player.backto_jurassic.step_num = player.backto_jurassic.step_num + step_num	
	player.backto_jurassic.step_num = (player.backto_jurassic.step_num - 1) % step_max + 1

	local slotsConfigInfo = BacktoJurassicSlotsConfig[step_num]

	local winAmount = amount
	if (slotsConfigInfo.bet_num > 0)
	then
		winAmount = winAmount * slotsConfigInfo.bet_num
	end

	if (slotsConfigInfo.bet_multiple_num > 0)
	then
		winAmount = winAmount * slotsConfigInfo.bet_multiple_num
	end
	--amount * slotsConfigInfo.bet_num * 

	if (player.backto_jurassic.step_num < old_step_num)
	then
		winAmount = winAmount + amount * 5
	end

	response.step_num = player.backto_jurassic.step_num
	...
	local notice = {
		header = {
			...
			message_id = "SlotsBacktoJurassicContest_Slots_Notice",
		},
		seat = {
			id = seat.id,
			online = seat.online,
		},
		step_num = player.backto_jurassic.step_num
	}
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsBacktoJurassicContest" then
		for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
				LOG(RUN, INFO).Format("[SlotsBacktoJurassicContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
	end
	response.ret = Return.OK()
...
SlotsBacktoJurassicContainer = {
	rooms = Container:Get("SlotsBacktoJurassic.rooms"),
	player_seat = Container:Get("SlotsBacktoJurassic.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
