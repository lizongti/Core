module("SlotsBruceLeeContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsBruceLeeContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.BruceLee)
	then
		local designated_channel_id = request.player.bruce_lee.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsBruceLeeContainer:MakeDesignatedSit(SlotsBruceLeeContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsBruceLeeContainer:MakeSit(SlotsBruceLeeContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsBruceLeeContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsBruceLeeContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsBruceLeeContainer:MakeStand(player_id) then
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsBruceLeeContest" then
...
SlotsBruceLeeContainer = {
	rooms = Container:Get("SlotsBruceLee.rooms"),
	player_seat = Container:Get("SlotsBruceLee.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
