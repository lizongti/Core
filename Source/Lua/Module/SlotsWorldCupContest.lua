module("SlotsWorldCupContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsWorldCupContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.WorldCup)
	then
		local designated_channel_id = request.player.world_cup.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsWorldCupContainer:MakeDesignatedSit(SlotsWorldCupContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsWorldCupContainer:MakeSit(SlotsWorldCupContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsWorldCupContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsWorldCupContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsWorldCupContainer:MakeStand(player_id) then
...
Offline = function ( _M, session, request )
	...
	LOG(RUN, INFO).Format("[SlotsWorldCupContest][Offline] player %s offline table %s!", player_id, channel_id)
	return response
end
...
Slots = function ( _M, session, request )
	...
	local step_max = #WorldCupSlotsConfig
	local old_step_num = player.world_cup.step_num
	player.world_cup.step_num = player.world_cup.step_num + step_num	
	player.world_cup.step_num = (player.world_cup.step_num - 1) % step_max + 1

	local slotsConfigInfo = WorldCupSlotsConfig[step_num]
	...
	if (player.world_cup.step_num < old_step_num)
	then
		winAmount = winAmount + amount * 5
	end

	response.step_num = player.world_cup.step_num
	...
	local notice = {
		header = {
			...
			message_id = "SlotsWorldCupContest_Slots_Notice",
		},
		seat = {
			id = seat.id,
			online = seat.online,
		},
		step_num = player.world_cup.step_num
	}
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsWorldCupContest" then
		for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
				LOG(RUN, INFO).Format("[SlotsWorldCupContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
	end
	response.ret = Return.OK()
...
SlotsWorldCupContainer = {
	rooms = Container:Get("SlotsWorldCup.rooms"),
	player_seat = Container:Get("SlotsWorldCup.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
