module("SlotsPirateContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	local seat = nil
	local game_room_config = GameRoomConfig[GameType.AllTypes.Pirate]

	SlotsPirateContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.Pirate)
	then
		local designated_channel_id = request.player.pirate.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsPirateContainer:MakeDesignatedSit(SlotsPirateContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsPirateContainer:MakeSit(SlotsPirateContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsPirateContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsPirateContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsPirateContainer:MakeStand(player_id) then
...
Offline = function ( _M, session, request )
	...
	local player_id = request.player_id
	LOG(RUN, INFO).Format("[SlotsPirateContest][Offline] player %s offline", player_id)
	local seat = SlotsPirateContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	...
	LOG(RUN, INFO).Format("[SlotsPirateContest][Offline] player %s offline table %s!", player_id, channel_id)
	return response
end
...
Slots = function ( _M, session, request )
	...
	local player_id = request.player.id
	local step_num = request.player.pirate.step_num

	seat.player.pirate.step_num = step_num
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsPirateContest" then
		for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
				LOG(RUN, INFO).Format("[SlotsPirateContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
	end
	response.ret = Return.OK()
...
SlotsPirateContainer = {
	rooms = Container:Get("SlotsPirate.rooms"),
	player_seat = Container:Get("SlotsPirate.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
