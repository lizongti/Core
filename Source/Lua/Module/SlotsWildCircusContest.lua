module("SlotsWildCircusContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsWildCircusContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.WildCircus)
	then
		local designated_channel_id = request.player.wild_circus.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsWildCircusContainer:MakeDesignatedSit(SlotsWildCircusContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsWildCircusContainer:MakeSit(SlotsWildCircusContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsWildCircusContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsWildCircusContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsWildCircusContainer:MakeStand(player_id) then
		response.ret = Return.CHEFS_CHOICE_CONTEST_NULL_EXIT()
		return response
	end
...
Slots = function ( _M, session, request )
	...
	local seat = SlotsWildCircusContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.CHEFS_CHOICE_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end
	...
	local step_max = #WildCircusSlotsConfig
	local old_step_num = player.wild_circus.step_num
	player.wild_circus.step_num = player.wild_circus.step_num + step_num	
	player.wild_circus.step_num = (player.wild_circus.step_num - 1) % step_max + 1

	local slotsConfigInfo = WildCircusSlotsConfig[step_num]
	...
	if (player.wild_circus.step_num < old_step_num)
	then
		winAmount = winAmount + amount * 5
	end

	response.step_num = player.wild_circus.step_num
	...
	local notice = {
		header = {
			...
			message_id = "SlotsWildCircusContest_Slots_Notice",
		},
		seat = {
			id = seat.id,
			online = seat.online,
		},
		step_num = player.wild_circus.step_num
	}
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsWildCircusContest" then
...
SlotsWildCircusContainer = {
	rooms = Container:Get("SlotsWildCircus.rooms"),
	player_seat = Container:Get("SlotsWildCircus.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
