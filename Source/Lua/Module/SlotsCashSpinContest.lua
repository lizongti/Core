module("SlotsCashSpinContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsCashSpinContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.CashSpin)
	then
		local designated_channel_id = request.player.cash_spin.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsCashSpinContainer:MakeDesignatedSit(SlotsCashSpinContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsCashSpinContainer:MakeSit(SlotsCashSpinContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsCashSpinContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsCashSpinContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsCashSpinContainer:MakeStand(player_id) then
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsCashSpinContest" then
...
SlotsCashSpinContainer = {
	rooms = Container:Get("SlotsCashSpin.rooms"),
	player_seat = Container:Get("SlotsCashSpin.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
