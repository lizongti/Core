module("SlotsLegendsofOlympusContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsLegendsofOlympusContainer:Init():NewRoom(game_room_config.room_name)

	if (request.player.game_type and request.player.game_type == GameType.AllTypes.LegendsofOlympus)
	then
		local designated_channel_id = request.player.legendsof_olympus.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsLegendsofOlympusContainer:MakeDesignatedSit(SlotsLegendsofOlympusContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsLegendsofOlympusContainer:MakeSit(SlotsLegendsofOlympusContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsLegendsofOlympusContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsLegendsofOlympusContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsLegendsofOlympusContainer:MakeStand(player_id) then
...
Offline = function ( _M, session, request )
	...
	local player_id = request.player_id
	LOG(RUN, INFO).Format("[SlotsLegendsofOlympusContest][Offline] player %s offline", player_id)
	local seat = SlotsLegendsofOlympusContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	...
	LOG(RUN, INFO).Format("[SlotsLegendsofOlympusContest][Offline] player %s offline table %s!", player_id, channel_id)
	return response
end
...
Hotfix = function(_M, session, request)
	...
	if process_name == "SlotsLegendsofOlympusContest" then
		for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
				LOG(RUN, INFO).Format("[SlotsLegendsofOlympusContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
	end
	response.ret = Return.OK()
...
SlotsLegendsofOlympusContainer = {
	rooms = Container:Get("SlotsLegendsofOlympus.rooms"),
	player_seat = Container:Get("SlotsLegendsofOlympus.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
