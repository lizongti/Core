require "Common/SlotsAliceinWonderlandConstCal"
require "Common/LineNum"
module("SlotsAliceinWonderlandContest", package.seeall)
local GlobalSession = {}
Enter = function(_M, session, request)
    ...
    SlotsAliceinWonderlandContainer:Init():NewRoom(game_room_config.room_name)

    if (request.player.game_type and request.player.game_type == GameType.AllTypes.AliceinWonderland) then
        local designated_channel_id = request.player.alicein_wonderland.channel_id
        local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
        table_id = tonumber(table_id)
        local designated_seat = SlotsAliceinWonderlandContainer:MakeDesignatedSit(SlotsAliceinWonderlandContainer[game_room_config.room_name], player_id, table_id)
        ...
        seat = SlotsAliceinWonderlandContainer:MakeSit(SlotsAliceinWonderlandContainer[game_room_config.room_name], player_id)
    end
    ...
    local channel_id = string.format("%s.%s.%s", "SlotsAliceinWonderlandContest", seat.table.room.name, seat.table.id)
    response.channel_id = channel_id
    response.table = {seat = {}}
    for i, seat in pairs(seat.table) do
        table.insert(response.table.seat, {id = seat.id, player = seat.player, online = seat.online,})
    end
...
Exit = function(_M, session, request)
    ...
    local seat = SlotsAliceinWonderlandContainer:LocateSeat(player_id)
    if not seat then
        response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
        return response
    end

    if not SlotsAliceinWonderlandContainer:MakeStand(player_id) then
        response.ret = Return.AliceinWonderland_CONTEST_NULL_EXIT()
        return response
    end
...
Offline = function(_M, session, request)
    ...
    local seat = SlotsAliceinWonderlandContainer:LocateSeat(player_id)
    if not seat then
        response.ret = Return.ALICEINWONDERLAND_CONTEST_CANNOT_FIND_PLAYER()
        return response
    end
    ...
    response = {header = response.header, ret = Return.OK()}

    return response
end

Award = function(_M, session, request)
    local response = {
        header = {
            router = "AsyncResponse",
            client_id = session.client_id,
    ...
    local selIndex = request.amount

    local GJackpotK = "AliceWonderJackpot.Amount" .. selIndex

    local curValue = GlobalState:GetLatest(GJackpotK)
    curValue = curValue / NUM_FLAG

    if (not curValue) then
        curValue = 0
    end
    response = {header = response.header, amount = curValue}
...
Hotfix = function(_M, session, request)
    ...
    if process_name == "SlotsAliceinWonderlandContest" then
...
SlotsAliceinWonderlandContainer = {
    rooms = Container:Get("SlotsAliceinWonderland.rooms"),
    player_seat = Container:Get("SlotsAliceinWonderland.player_seat"),
    Init = function(self)
        setmetatable(self, {__index = self.rooms})
        return self
    end,
    ...
    NewRoom = function(self, name)
        ...
        local meta = {free_seats = {}, name = name, max_table = 0,}
        meta.meta = meta
        setmetatable(new_room, {__index = meta})
        return new_room
    end,
    ...
    NewTable = function(self, room)
        ...
        local new_table = {}
        local meta = {room = room, id = room.max_table}
        meta.meta = meta
        setmetatable(new_table, {__index = meta})
        for i = 1, 5, 1 do
    ...
    NewSeat = function(self, table, id)
        local new_seat = {}
        local meta = {table = table, id = id}
        meta.meta = meta
        setmetatable(new_seat, {__index = meta})
    ...
    MakeDesignatedSit = function(self, room, player_id, table_id)
        ...
        for index, seat in pairs(room.free_seats) do
            if (room.free_seats[index].table.id == table_id) then
