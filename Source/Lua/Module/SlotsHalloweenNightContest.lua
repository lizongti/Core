module("SlotsHalloweenNightContest", package.seeall)
...
Enter = function ( _M, session, request )
	...
	SlotsHalloweenNightContainer:Init():NewRoom(game_room_config.room_name)
	
	if (request.player.game_type and request.player.game_type == GameType.AllTypes.HalloweenNight)
	then
		local designated_channel_id = request.player.halloween_night.channel_id
		local contest_id, room_id, table_id = unpack(string.split(designated_channel_id, "."))
		table_id = tonumber(table_id)
		local designated_seat = SlotsHalloweenNightContainer:MakeDesignatedSit(SlotsHalloweenNightContainer[game_room_config.room_name], player_id, table_id)
		...
		seat = SlotsHalloweenNightContainer:MakeSit(SlotsHalloweenNightContainer[game_room_config.room_name], player_id)
	end
	...
	local channel_id = string.format("%s.%s.%s", "SlotsHalloweenNightContest", seat.table.room.name, seat.table.id)
...
Exit = function ( _M, session, request )
	...
	local seat = SlotsHalloweenNightContainer:LocateSeat(player_id)
	if not seat then
		response.ret = Return.GAME_CONTEST_CANNOT_FIND_PLAYER()
		return response
	end

	if not SlotsHalloweenNightContainer:MakeStand(player_id) then
		response.ret = Return.HALLOWEENNIGTH_CONTEST_NULL_EXIT()
		return response
	end
...
-- 	for room_id, room in pairs(SlotsHalloweenNightContainer.rooms) do
...
--                     if SlotsHalloweenNightContainer:MakeStand(player_id) then
--                         local notice = {
--                             header = {
...
Offline = function ( _M, session, request )
	...
	LOG(RUN, INFO).Format("[SlotsContest][Offline] player %s offline table %s!", player_id, channel_id)
	return response
end
...
Hotfix = function(_M, session, request)
    ...
    if process_name == "SlotsHalloweenNightContest" then
        for i = 1, #module_name do
			...
			if package.loaded[full_path] then
				...
				require (full_path)
                LOG(RUN, INFO).Format("[SlotsHalloweenNightContest][Hotfix] successfully hotfixed %s", full_path)
			end
		end
    end
    response.ret = Return.OK()
...
SlotsHalloweenNightContainer = {
	rooms = Container:Get("SlotsHalloweenNight.rooms"),
	player_seat = Container:Get("SlotsHalloweenNight.player_seat"),
	Init = function(self)
		setmetatable(self, {
			__index = self.rooms
		})
