...
module("SlotsDragonTale", package.seeall)
...
Enter = function(_M, session, request)
    ...
    local filter_ret = RequestFilter.Filter("SlotsDragonTale", "Enter", session, request, true)
    if filter_ret then
    ...
    local player_slots_info = CommonCal.Calculate.get_slots_info(session, task, player, GameType.AllTypes.DragonTale)
    local dragon_tale = json.decode(player_slots_info.content)

    CommonCal.Calculate.MakeUpInRoom(session, task)
    local isLock = CommonCal.Calculate.LevelReq(player, GameType.AllTypes.DragonTale)
    if (isLock == 1) then
        response.ret = Return.LOCK_GAME()
        return response
    end

    local game_room_config = GameRoomConfig[GameType.AllTypes.DragonTale]

    local table_id = request.table_id
    local async_request = nil
    if (table_id and table_id > 0) then
        local channel_id = string.format("%s.%s.%s", "SlotsDragonTaleContest", game_room_config.room_name, table_id)
        dragon_tale.channel_id = channel_id
        async_request = {
            header = {
                router = "AsyncRequest",
                service_name = game_room_config.contest_client_name,
                task_id = task.id,
                module_id = "SlotsDragonTaleContest",
                message_id = "SlotsDragonTaleContest_Enter_Request"
            },
            player = {
                id = player.id,
                user = player.user,
                account = player.account,
                game_type = GameType.AllTypes.DragonTale,
                character = {
    ...
    LOG(RUN, INFO).Format("[SlotsDragonTale][Enter] player %s", player.id)
    ...
    dragon_tale.channel_id = async_response.channel_id
    ...
    dragon_tale.bouts_id = 0
     --进场的时候把cd清空掉
    dragon_tale.enter_chip = player.character.chip
    dragon_tale.spined_times = 0
    ...
    player.game_type = GameType.AllTypes.DragonTale

    -----------------紧急修复-------------------
    if (dragon_tale.free_spin_bouts > 50) then
        dragon_tale.free_spin_bouts = 0
    end

    player_slots_info.content = json.encode(dragon_tale)
...
Start = function(_M, session, request)
    ...
    local filter_ret = RequestFilter.Filter("SlotsDragonTale", "Start", session, request, true)
    if filter_ret then
        response.ret = filter_ret
        return response
    end
    ...
    if (player.game_type ~= GameType.AllTypes.DragonTale) then
    ...
    local is_free_spin = dragon_tale.free_spin_bouts > 0
    ...
    if is_free_spin then
        ...
        ------201810310936结束------------------
        amount = dragon_tale.bet_amount
        chipAmount = amount * 25
        dragon_tale.free_spin_bouts = math.max(dragon_tale.free_spin_bouts - 1, 0)
    else
        amount = request.amount
        chip_cost = amount * 25
        chipAmount = amount * 25
        if not Player:Consume(player, {"Chip", chip_cost}, Reason.DRAGONTALE_BET_CHIP_CONSUME()) then
            response.ret = Return.GAME_PLAYER_CHIP_NOT_ENOUGH()
            return response
        end
        dragon_tale.bet_amount = amount
    end
    ------201810310936开始------------------
    ...
    local origin_result, reel_file_name = SlotsDragonTaleCal.Calculate.GenItemResult(player, is_free_spin, false, nil)

    local origin_prize_items, origin_payrate = SlotsDragonTaleCal.Calculate.GenPrizeInfo(player, origin_result)
    table.insert(all_prize_items, origin_prize_items)
    local free_spin_bouts = SlotsDragonTaleCal.Calculate.GenFreeSpinCount(player, origin_result)
    local total_payrate
    local total_win_chip = 0
    --如果中奖则不会进行冰龙火龙的判定,如果不中奖则会
    local need_respin = SlotsDragonTaleCal.Calculate.NeedRespin(player)
    ...
    if (origin_payrate == 0 and free_spin_bouts == 0 and need_respin) then
        ...
        rep_result = SlotsDragonTaleCal.Calculate.ReplaceSideLinesToWild(origin_result)

        local rep_prize_items, rep_payrate = SlotsDragonTaleCal.Calculate.GenPrizeInfo(player, rep_result)
        table.insert(all_prize_items, rep_prize_items)
        response.prize_items = rep_prize_items
        response.win_chip = rep_payrate * amount

        local dragons = SlotsDragonTaleCal.Calculate.GenDragonType(player)
        CommonCal.Calculate.SetLoopNum(player.id, 2)

        second_result, reel_file_name = SlotsDragonTaleCal.Calculate.GenItemResult(player, is_free_spin, true, nil)

        second_result = SlotsDragonTaleCal.Calculate.ReplaceSideLinesToWild(second_result)

        local respin_prize_items, respin_payrate = SlotsDragonTaleCal.Calculate.GenPrizeInfo(player, second_result)
        table.insert(all_prize_items, respin_prize_items)

        free_spin_bouts = SlotsDragonTaleCal.Calculate.GenFreeSpinCount(player, second_result)
        -- free_spin_bouts = 10
        response.respin = 1
        response.dragons = dragons
        response.final_item_ids = SlotsDragonTaleCal.Calculate.TransResultToList(second_result)
    ...
    response.item_ids = SlotsDragonTaleCal.Calculate.TransResultToList(origin_result)
    ...
    dragon_tale.free_spin_bouts = dragon_tale.free_spin_bouts + free_spin_bouts

    if (not is_free_spin and is_get_free_spin) then
        dragon_tale.bouts_id = os.time()
    end
    ...
    if total_win_chip > 0 then
        Player:Obtain(player, {"Chip", total_win_chip}, Reason.DRAGONTALE_BET_CHIP_OBTAIN())
        ...
        RankHelper:ChallengeDailyBiggestWin(player, GameType.AllTypes.DragonTale, rep_data)
        RankHelper:ChallengeWeeklyBiggestWin(player, GameType.AllTypes.DragonTale, rep_data)
        --end
        ----记录record
    ...
    if (response.respin > 0) then
        ...
        Spark:FeatureSpecialWin(
            player,
            {
                [1] = GameType.AllTypes.DragonTale,
                [2] = "DragonTale",
    ...
    -----记录free spin中总赢取
    if is_free_spin then
        dragon_tale.free_total_win = dragon_tale.free_total_win + total_win_chip
    else
        dragon_tale.free_total_win = 0
    end
    ...
    local DragonTalePrizeConfig = CommonCal.Calculate.get_config(player, "DragonTalePrizeConfig")
    ...
    local task_req_data = {
        ...
        max_bet = amount >= SlotsDragonTaleCal.Calculate.GetMaxBetAmount(player),
        free_win_amount = free_win_amount,
        epic_win = (total_win_chip / chipAmount) >= DragonTalePrizeConfig[3].min_multiple
    }
    ...
    if ((total_win_chip / (amount * 25)) >= DragonTalePrizeConfig[3].min_multiple) then
        player.statistics.epicwin_num = player.statistics.epicwin_num + 1
    elseif ((total_win_chip / (amount * 25)) >= DragonTalePrizeConfig[2].min_multiple) then
        player.statistics.megawin_num = player.statistics.megawin_num + 1
    elseif ((total_win_chip / (amount * 25)) >= DragonTalePrizeConfig[1].min_multiple) then
    ...
    if (can_multiply) then
        if ((total_win_chip / chipAmount) >= DragonTalePrizeConfig[1].min_multiple and not is_free_spin) then
    ...
    Communication:OnBcEvent(session, BroadcastType.AllTypes.DragonTaleSpin, win_info, 15)

    --opt
    local contest_id, room_id, table_id = unpack(string.split(dragon_tale.channel_id, "."))
    if (is_free_spin) then
        Spark:SlotsAward(
            player,
            {
                ...
                [2] = "DragonTale",
                [3] = table_id,
                [4] = dragon_tale.bouts_id,
                [5] = dragon_tale.bet_amount,
                [6] = total_win_chip,
                [7] = json.encode(origin_result),
                [8] = json.encode(response.final_item_ids),
                [9] = reel_file_name,
                [10] = dragon_tale.free_spin_bouts,
    ...
    --buyloss的局数+1
    dragon_tale.spined_times = dragon_tale.spined_times + 1

    ------201810310936开始------------------
    CommonCal.Calculate.EndStart(session, task, player, request, response, dragon_tale, 25, chip_cost, total_win_chip)

    --gain exp
    if not is_free_spin then
        local exp = chip_cost
        local exp_request = {
            type = "SlotsDragonTale",
            ante_gold = amount * 25,
            gain_exp = exp
        }
    ...
    response.player = {
        ...
        dragon_tale = {
            bet_amount = amount,
            free_spin_bouts = dragon_tale.free_spin_bouts,
            free_total_win = dragon_tale.free_total_win
        }
    }
...
Exit = function(_M, session, request)
    local response = {header = {router = "Response"}}
    local filter_ret = RequestFilter.Filter("SlotsDragonTale", "Exit", session, request, true)
    if filter_ret then
        response.ret = filter_ret
        return response
    end
    ...
    local async_request = {
        header = {
            ...
            task_id = task.id,
            module_id = "SlotsDragonTaleContest",
            message_id = "SlotsDragonTaleContest_Exit_Request"
        },
        player_id = player.id
    }
    ...
    local trigger_buyloss, total_loss, diamond, goods_id =
        BuyLoss:Trigger(session, task, GameType.AllTypes.DragonTale, player)
    if trigger_buyloss then
        dragon_tale.total_loss = total_loss
    ...
    LOG(RUN, INFO).Format("[SlotsDragonTale][Exit] ok player %s", player.id)
    return response
end
