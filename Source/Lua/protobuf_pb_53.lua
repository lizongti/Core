...
function _R_meta:__index(key)
	if not decode_type_cache[self._CType][key] then
		return
	end
...
function _encode_type_meta:__index(key)
	local t, msg = c._env_type(P, self._CType, key)
	if not _writer[t] or not key then
		return
	end
...
function M.decodeAll(typename, buffer)
   local ret = M.decode(typename, buffer, length)
   if ret then
      M.decodeTable(ret)
   end
   return ret
...
function M.decodeTable(tbl)
   for k,v in pairs(tbl) do
       if type(v) == "table" then
          if type(v[1]) == "string" then
               local ret = M.decode(v[1], v[2])
               if ret then
                  tbl[k] = ret
               end
          end
