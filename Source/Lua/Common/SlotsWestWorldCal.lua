module("SlotsWestWorldCal", package.seeall)
...
Calculate = {
    ...
    GetMaxBetAmount = function(player)
        ...
        for k, v in ipairs(WestWorldBetAmountConfig)
        do
            if (player.character.level >= old_req_level and player.character.level < v.required_level)
            then
                return WestWorldBetAmountConfig[k - 1].single_amount
            end
            old_req_level = v.required_level
        end

        local max_index = #WestWorldBetAmountConfig
		if (player.character.level >= WestWorldBetAmountConfig[max_index].required_level)
		then
			return WestWorldBetAmountConfig[max_index].single_amount
		end
    ...
    CompareNormalAndWild = function (player, item_id, continue_count, wild_count, prize_list, direction )
        local WestWorldPayrateConfig = CommonCal.Calculate.get_config(player, "WestWorldPayrateConfig")
		...
		if has_wild_prize and has_normal_prize then
            ...
            if (WestWorldPayrateConfig[item_id] and WestWorldPayrateConfig[item_id].payrate)
            then
                normal_payrate = WestWorldPayrateConfig[item_id].payrate[continue_count - 2]
            end
            local wild_payrate = 0
            if (WestWorldPayrateConfig[Const.Types.Wild] and WestWorldPayrateConfig[Const.Types.Wild].payrate)
            then
                wild_payrate = WestWorldPayrateConfig[Const.Types.Wild].payrate[wild_count - 2]
            end
        ...
        --LOG(RUN, INFO).Format("[SlotsWestWorld][GenOneLinePrize] prize_list: %s", Table2Str(prize_list))
	end,
    ...
    --        end
    --    end
    --    LOG(RUN, INFO).Format("[SlotsWestWorld][GenOneLinePrize] rep_line_data: %s", Table2Str(rep_line_data))
    --    local item_id, continue_count = Calculate.GenNormalContinueCount(rep_line_data)
    --    if item_id and continue_count then
    --        LOG(RUN, INFO).Format("[SlotsWestWorld][GenOneLinePrize] wild_count is: %s, item_id is: %s, continue_count is: %s", wild_count, item_id, continue_count)
    --        table.insert(prize_list, {
    --            continue_count = continue_count,
    --            item_id = item_id,
    --        })
    ...
    GenFreeSpinCount = function(player, data)
        ...
        local WestWorldOthersConfig = CommonCal.Calculate.get_config(player, "WestWorldOthersConfig")
        return (scatter_count >= 3) and WestWorldOthersConfig[1].free_spin_delta or 0
    end,
		...
		for line_index, v in ipairs(Const.Lines) do
           ...
           -- then
			...
			for _, item in ipairs(one_line_prize) do
				...
				local config = WestWorldPayrateConfig[item.item_id]
				if (config ~= nil)
				then
...
--产生一列,column表示第几列,config是经过了is_free_spin判断了之后的config
    ...
    GenItemResult = function (player, is_free_spin, is_final, feature_file)
        ...
        local reel_file_name = "WestWorld" .. base_or_feature .. "Reel" .. reel_index .. "Config"

        if (not is_final)
        then
            if (feature_file ~= nil and feature_file == "WestWorldMFReelConfig")
            then
		...
		then		
			reel_file_name = "WestWorldNewHand1" .. base_or_feature .. "Reel" .. reel_index .. "Config"
		elseif (player.character.level <= tonumber(ConstValue[9].value))
		then
			reel_file_name = "WestWorldNewHand2" .. base_or_feature .. "Reel" .. reel_index .. "Config"
		elseif (player.character.level <= tonumber(ConstValue[10].value))
		then
			reel_file_name = "WestWorldNewHand3" .. base_or_feature .. "Reel" .. reel_index .. "Config"
        end

        LOG(RUN, INFO).Format("[SlotsWestWorldCal][GenItemResult] player %s reel_file_name is:%s", player.id, reel_file_name)
...
--trans 5*3 matrix to 3*5 matrix
    ...
    NeedRespin = function(player)
        ...
        local prob = is_free_spin and WestWorldOthersConfig[1].free_trigger_wild_prob or WestWorldOthersConfig[1].trigger_wild_prob
