module("FastConfig", package.seeall)

New = function(self, base_config, fast_config)
	setmetatable(
		self,
		{
			__index = base_config
...
LoggerService = function(self)
	...
	for _, log_type in ipairs(log_types) do
		table.insert(
			self.config.LoggerService.logs,
			{
...
ManagerService = function(self, concurrency)
	...
	for index = 0, concurrency - 1, 1 do
		table.insert(
			self.config.ManagerService.zeromqs,
			{
...
NotificationService = function(self, concurrency)
	concurrency = concurrency or self.fast_config.concurrency
	...
	for index = 0, concurrency - 1, 1 do
		table.insert(
			self.config.NotificationService.zeromqs,
			{
...
CacheIntegratedService = function(self, concurrency)
	-- concurrency = concurrency or self.fast_config.concurrency
	...
	for index = 0, concurrency - 1, 1 do
		table.insert(
			self.config.CacheIntegratedService.lines,
			{
				redis = {
...
DatabaseIntegratedService = function(self, concurrency)
	-- concurrency = concurrency or self.fast_config.concurrency
	concurrency = 1 -- 和HeartBeat.lua的遍历个数对应
	...
	for index = 0, concurrency - 1, 1 do
		table.insert(
			self.config.DatabaseIntegratedService.lines,
			{
				mysql = {
...
ManagerClientService = function(self, index)
...
NotificationClientService = function(self, index)
	index = index or self.fast_config.index
...
DispatcherService = function(self, index)
	index = index or self.fast_config.index
...
BackstageService = function(self)
