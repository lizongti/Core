...
class queue {
 ...
 public:
  ...
  value_type wait_and_pop() {
    ...
    condition_variable.wait(lock, [this] { return !this->queue_.empty(); });
...
template <typename T>
class shared_queue : public queue<std::shared_ptr<T>> {
 public:
...
class unique_shared_queue {
 public:
  ...
  static auto size()
      -> decltype(detail::singleton<shared_queue<T>>::instance().size()) {
